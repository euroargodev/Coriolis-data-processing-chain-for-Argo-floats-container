...
for idFloat = 1:nbFloats
   ...
   for idSensor = 1:length(sensorList)
      [sensorName, sensorDimLevel, sensorMaker, sensorModel, sensorSn] = ...
         get_sensor_info(sensorList{idSensor}, floatNum, floatDecId, metaWmoList, metaData);
...
switch a_decId
   ...
   case {228}
      % CTD 3T floats
      o_sensorList = [{'CTD'}; {'CTD2'}; {'CTD3'}];
      
   case {229}
      % CTD 2T floats
      o_sensorList = [{'CTD'}; {'CTD2'}];
...
function [o_sensorName, o_sensorDimLevel, o_sensorMaker, o_sensorModel, o_sensorSn] = ...
   get_sensor_info(a_inputSensorName, a_floatNum, a_decId, a_metaWmoList, a_metaData)
...
switch a_inputSensorName
   case  'CTD'
      if (~ismember(a_decId, g_decArgo_decoderIdListNkeIridiumRbr) || (a_decId == 228))
         o_paramName = [ ...
            {'PRES'} {'TEMP'} {'PSAL'} ...
            ];
