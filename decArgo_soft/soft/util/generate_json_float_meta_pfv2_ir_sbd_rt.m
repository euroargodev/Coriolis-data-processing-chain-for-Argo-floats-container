...
%
% SYNTAX :
%  generate_json_float_meta_pfv2_ir_sbd_rt(varargin)
%
% INPUT PARAMETERS :
...
%   'inputRsyncDataDirName' : directory of Iridium email files
%   'tempDirName'           : temporary directory used to decode received XML settings files
...
% ------------------------------------------------------------------------------
% RELEASES :
%   10/24/2025 - RNU - creation
% ------------------------------------------------------------------------------
function generate_json_float_meta_pfv2_ir_sbd_rt(varargin)
...
global g_cogj_inputRsyncDataDirName;
global g_cogj_tempDirName;
...
try
   ...
   if (~isempty(g_cogj_outputLogDirName))
      logFileName = [g_cogj_outputLogDirName '/generate_json_float_meta_pfv2_ir_sbd_rt_' currentTime '.log'];
   else
      logFileName = [tempdir '/generate_json_float_meta_pfv2_ir_sbd_rt_' currentTime '.log'];
   end
...
if (~isempty(g_cogj_xmlReportDirName))
   xmlFileName = [g_cogj_xmlReportDirName '/generate_json_float_meta_pfv2_ir_sbd_rt_' currentTime '.xml'];
else
   xmlFileName = [tempdir '/generate_json_float_meta_pfv2_ir_sbd_rt_' currentTime '.xml'];
end
...
newChild.appendChild(docNode.createTextNode('Argo Coriolis float meta-data json file generating tool (generate_json_float_meta_pfv2_ir_sbd_rt)'));
docRootNode.appendChild(newChild);
...
if (~isempty(a_varargin))
   if (rem(length(a_varargin), 2) ~= 0)
      ...
      for id = 1:2:length(a_varargin)
         ...
         elseif (strcmpi(a_varargin{id}, 'inputRsyncDataDirName'))
            g_cogj_inputRsyncDataDirName = a_varargin{id+1};
         elseif (strcmpi(a_varargin{id}, 'tempDirName'))
            g_cogj_tempDirName = a_varargin{id+1};
...
if (isempty(g_cogj_rbrMetaDataDirName))
   o_logLines{end+1} = sprintf('ERROR: ''rbrMetaDataDirName'' input parameter is mandatory\n');
   o_inputError = 1;
   return
end
...
if (isempty(g_cogj_inputRsyncDataDirName))
   o_logLines{end+1} = sprintf('ERROR: ''inputRsyncDataDirName'' input parameter is mandatory\n');
   o_inputError = 1;
   return
end
if (isempty(g_cogj_tempDirName))
   o_logLines{end+1} = sprintf('ERROR: ''tempDirName'' input parameter is mandatory\n');
   o_inputError = 1;
   return
end
if (isempty(g_cogj_xmlReportDirName))
...
o_logLines{end+1} = sprintf('inputRsyncDataDirName: %s\n', g_cogj_inputRsyncDataDirName);
o_logLines{end+1} = sprintf('tempDirName: %s\n', g_cogj_tempDirName);
