...
% ------------------------------------------------------------------------------
% RELEASES :
%   01/28/2025 - RNU - creation
% ------------------------------------------------------------------------------
function [o_metaStruct] = get_meta_data_cts5_140(...
   a_metaDataXmlFileName, a_metaStruct, a_sensorListNum, a_floatNum, a_outputCsvDirName, a_rtVersionFlag)
...
if (isfield(metaData, 'telecom'))
   ...
   if (~a_rtVersionFlag)
      if (isfield(metaData.telecom, 'imei'))
         if (~strcmp(metaData.telecom.imei, o_metaStruct.IMEI))
            if (isempty(g_cogj_csvFileBddId) || (g_cogj_csvFileBddId == -1))
               [g_cogj_csvFileBddId, g_cogj_csvFileBddPathName] = open_csv_file(a_outputCsvDirName, 'B', g_cogj_csvFileBddPathName);
            end
            fprintf('WARNING: BDD: Float #%d: IMEI differ (database: ''%s'' xml:''%s'') - DB contents should be updated (see %s)\n', ...
               a_floatNum, ...
               o_metaStruct.IMEI, ...
               metaData.telecom.imei, ...
               g_cogj_csvFileBddPathName);
            fprintf(g_cogj_csvFileBddId, '%d;1243;1;%s;IMEI\n', ...
               a_floatNum, ...
               metaData.telecom.imei);
         end
      end
      ...
      if (isfield(metaData.hardware, 'control_board'))
         ...
         if (isfield(metaData.hardware.control_board, 'firmware'))
            if (~strcmp(metaData.hardware.control_board.firmware, o_metaStruct.FIRMWARE_VERSION))
               ...
               fprintf('WARNING: BDD: Float #%d: CONTROLLER_BOARD_FIRMWARE_VERSION_PRIMARY differ (database: ''%s'' xml:''%s'') - DB contents should be updated (see %s)\n', ...
                  a_floatNum, ...
...
if (isfield(metaData, 'sensors'))
   ...
   if (any(a_sensorListNum == 4))
      if (~a_rtVersionFlag)
         ...
         elseif (any(strcmp(o_metaStruct.SENSOR_MOUNTED_ON_FLOAT, 'ECO_FLNTU')))
            % ECO_FLNTU
            if (isfield(metaData.sensors, 'sensor_eco'))
               idF = find(strcmp('FLUOROMETER_CHLA', o_metaStruct.SENSOR));
               idF2 = find(strcmp('BACKSCATTERINGMETER_TURBIDITY', o_metaStruct.SENSOR));
               if (~isempty(idF) && ~isempty(idF2))
                     ...
                     if (isfield(metaData.sensors.sensor_eco.sensor, 'sn'))
                        if (str2double(metaData.sensors.sensor_eco.sensor.sn) ~= str2double(o_metaStruct.SENSOR_SERIAL_NO{idF}))
                           ...
                           fprintf('WARNING: BDD: Float #%d: SENSOR_SERIAL_NO for ''%s'' differ (database: ''%s'' xml:''%s'') - DB contents should be updated (see %s)\n', ...
                              ...
                              metaData.sensors.sensor_eco.sensor.sn, ...
                              g_cogj_csvFileBddPathName);
                           fprintf(g_cogj_csvFileBddId, '%d;411;305;%s;SENSOR_SERIAL_NO\n', ...
                              a_floatNum, ...
                              metaData.sensors.sensor_eco.sensor.sn);
                        end
                     end
                     if (isfield(metaData.sensors.sensor_eco.sensor, 'model'))
                        metaData.sensors.sensor_eco.sensor.model = 'ECO_FLNTU'; % because it is 'ECO2' in NKE XML file
                        if (~strcmp(metaData.sensors.sensor_eco.sensor.model, o_metaStruct.SENSOR_MODEL{idF}))
                           if (isempty(g_cogj_csvFileBddId) || (g_cogj_csvFileBddId == -1))
                              [g_cogj_csvFileBddId, g_cogj_csvFileBddPathName] = open_csv_file(a_outputCsvDirName, 'B', g_cogj_csvFileBddPathName);
                           end
                           ...
                           fprintf('WARNING: BDD: Float #%d: SENSOR_MODEL for ''%s'' differ (database: ''%s'' xml:''%s'') - DB contents should be updated (see %s)\n', ...
                              ...
                              metaData.sensors.sensor_eco.sensor.model, ...
                              g_cogj_csvFileBddPathName);
                           fprintf(g_cogj_csvFileBddId, '%d;410;305;%s;SENSOR_MODEL\n', ...
                              a_floatNum, ...
                              metaData.sensors.sensor_eco.sensor.model);
                        end
                     end
                  ...
                  if (isfield(metaData.sensors.sensor_eco, 'channel_01'))
                     if (isfield(metaData.sensors.sensor_eco.channel_01, 'sf'))
                        if (~isfield(o_metaStruct.CALIBRATION_COEFFICIENT, 'ECO_FLNTU'))
                           if (isempty(g_cogj_csvFileCoefId) || (g_cogj_csvFileCoefId == -1))
                              [g_cogj_csvFileCoefId, g_cogj_csvFileCoefPathName] = open_csv_file(a_outputCsvDirName, 'C', g_cogj_csvFileCoefPathName);
                           end
                           fprintf('WARNING: COEF: Float #%d: ECO_FLNTU ScaleFactChloroA coef (%s) is missing in META.json file - calib_coef file contents should be updated (see %s)\n', ...
                              a_floatNum, ...
                              metaData.sensors.sensor_eco.channel_01.sf, ...
                              g_cogj_csvFileCoefPathName);
                           fprintf(g_cogj_csvFileCoefId, '%d;ECO_FLNTU;ScaleFactChloroA;%s\n', ...
                              a_floatNum, ...
                              metaData.sensors.sensor_eco.channel_01.sf);
                        elseif (str2double(metaData.sensors.sensor_eco.channel_01.sf) ~= str2double(o_metaStruct.CALIBRATION_COEFFICIENT.ECO_FLNTU.ScaleFactChloroA))
                           if (isempty(g_cogj_csvFileCoefId) || (g_cogj_csvFileCoefId == -1))
                              [g_cogj_csvFileCoefId, g_cogj_csvFileCoefPathName] = open_csv_file(a_outputCsvDirName, 'C', g_cogj_csvFileCoefPathName);
                           end
                           fprintf('WARNING: COEF: Float #%d: ECO_FLNTU ScaleFactChloroA coef differ (database: ''%s'' xml:''%s'') - DB contents should be updated (see %s)\n', ...
                              a_floatNum, ...
                              o_metaStruct.CALIBRATION_COEFFICIENT.ECO_FLNTU.ScaleFactChloroA, ...
                              metaData.sensors.sensor_eco.channel_01.sf, ...
                              g_cogj_csvFileCoefPathName);
                           fprintf(g_cogj_csvFileCoefId, '%d;ECO_FLNTU;ScaleFactChloroA;%s\n', ...
                              a_floatNum, ...
                     ...
                     if (isfield(metaData.sensors.sensor_eco.channel_01, 'dc'))
                        if (~isfield(o_metaStruct.CALIBRATION_COEFFICIENT, 'ECO_FLNTU'))
                           ...
                           fprintf('WARNING: COEF: Float #%d: ECO_FLNTU DarkCountChloroA coef (%s) is missing in META.json file - calib_coef file contents should be updated (see %s)\n', ...
                              a_floatNum, ...
                              metaData.sensors.sensor_eco.channel_01.dc, ...
                              g_cogj_csvFileCoefPathName);
                           fprintf(g_cogj_csvFileCoefId, '%d;ECO_FLNTU;DarkCountChloroA;%s\n', ...
                              a_floatNum, ...
                              metaData.sensors.sensor_eco.channel_01.dc);
                        elseif (str2double(metaData.sensors.sensor_eco.channel_01.dc) ~= str2double(o_metaStruct.CALIBRATION_COEFFICIENT.ECO_FLNTU.DarkCountChloroA))
                           if (isempty(g_cogj_csvFileCoefId) || (g_cogj_csvFileCoefId == -1))
                              [g_cogj_csvFileCoefId, g_cogj_csvFileCoefPathName] = open_csv_file(a_outputCsvDirName, 'C', g_cogj_csvFileCoefPathName);
                           end
                           fprintf('WARNING: COEF: Float #%d: ECO_FLNTU DarkCountChloroA coef differ (database: ''%s'' xml:''%s'') - DB contents should be updated (see %s)\n', ...
                              a_floatNum, ...
                              o_metaStruct.CALIBRATION_COEFFICIENT.ECO_FLNTU.DarkCountChloroA, ...
                              metaData.sensors.sensor_eco.channel_01.dc, ...
                              g_cogj_csvFileCoefPathName);
                           fprintf(g_cogj_csvFileCoefId, '%d;ECO_FLNTU;DarkCountChloroA;%s\n', ...
                              a_floatNum, ...
                  ...
                  if (isfield(metaData.sensors.sensor_eco, 'channel_02'))
                     if (isfield(metaData.sensors.sensor_eco.channel_02, 'sf'))
                        if (~isfield(o_metaStruct.CALIBRATION_COEFFICIENT, 'ECO_FLNTU'))
                           ...
                           fprintf('WARNING: COEF: Float #%d: ECO_FLNTU ScaleFactTurbi coef (%s) is missing in META.json file - calib_coef file contents should be updated (see %s)\n', ...
                              a_floatNum, ...
                              metaData.sensors.sensor_eco.channel_02.sf, ...
                              g_cogj_csvFileCoefPathName);
                           fprintf(g_cogj_csvFileCoefId, '%d;ECO_FLNTU;ScaleFactTurbi;%s\n', ...
                              a_floatNum, ...
                              metaData.sensors.sensor_eco.channel_02.sf);
                        elseif (str2double(metaData.sensors.sensor_eco.channel_02.sf) ~= str2double(o_metaStruct.CALIBRATION_COEFFICIENT.ECO_FLNTU.ScaleFactTurbi))
                           if (isempty(g_cogj_csvFileCoefId) || (g_cogj_csvFileCoefId == -1))
                              [g_cogj_csvFileCoefId, g_cogj_csvFileCoefPathName] = open_csv_file(a_outputCsvDirName, 'C', g_cogj_csvFileCoefPathName);
                           end
                           fprintf('WARNING: COEF: Float #%d: ECO_FLNTU ScaleFactTurbi coef differ (database: ''%s'' xml:''%s'') - DB contents should be updated (see %s)\n', ...
                              a_floatNum, ...
                              o_metaStruct.CALIBRATION_COEFFICIENT.ECO_FLNTU.ScaleFactTurbi, ...
                              metaData.sensors.sensor_eco.channel_02.sf, ...
                              g_cogj_csvFileCoefPathName);
                           fprintf(g_cogj_csvFileCoefId, '%d;ECO_FLNTU;ScaleFactTurbi;%s\n', ...
                              a_floatNum, ...
                     ...
                     if (isfield(metaData.sensors.sensor_eco.channel_02, 'dc'))
                        if (~isfield(o_metaStruct.CALIBRATION_COEFFICIENT, 'ECO_FLNTU'))
                           ...
                           fprintf('WARNING: COEF: Float #%d: ECO_FLNTU DarkCountTurbi coef (%s) is missing in META.json file - calib_coef file contents should be updated (see %s)\n', ...
                              a_floatNum, ...
                              metaData.sensors.sensor_eco.channel_02.dc, ...
                              g_cogj_csvFileCoefPathName);
                           fprintf(g_cogj_csvFileCoefId, '%d;ECO_FLNTU;DarkCountTurbi;%s\n', ...
                              a_floatNum, ...
                              metaData.sensors.sensor_eco.channel_02.dc);
                        elseif (str2double(metaData.sensors.sensor_eco.channel_02.dc) ~= str2double(o_metaStruct.CALIBRATION_COEFFICIENT.ECO_FLNTU.DarkCountTurbi))
                           if (isempty(g_cogj_csvFileCoefId) || (g_cogj_csvFileCoefId == -1))
                              [g_cogj_csvFileCoefId, g_cogj_csvFileCoefPathName] = open_csv_file(a_outputCsvDirName, 'C', g_cogj_csvFileCoefPathName);
                           end
                           fprintf('WARNING: COEF: Float #%d: ECO_FLNTU DarkCountBackscatter700 coef differ (database: ''%s'' xml:''%s'') - DB contents should be updated (see %s)\n', ...
                              a_floatNum, ...
                              o_metaStruct.CALIBRATION_COEFFICIENT.ECO_FLNTU.DarkCountTurbi, ...
                              metaData.sensors.sensor_eco.channel_02.dc, ...
                              g_cogj_csvFileCoefPathName);
                           fprintf(g_cogj_csvFileCoefId, '%d;ECO_FLNTU;DarkCountTurbi;%s\n', ...
                              a_floatNum, ...
                  ...
                  fprintf('ERROR: Float #%d: cannot find expected ''FLUOROMETER_CHLA'' and/or ''BACKSCATTERINGMETER_TURBIDITY'' sensor in DB contents - ECO_FLNTU metadata ignored\n', ...
                     a_floatNum);
               end
            end
         else
   ...
   if (any(a_sensorListNum == 15))
      if (isfield(metaData.sensors, 'sensor_opus'))
         ...
         if (~isempty(idF))
            if (~a_rtVersionFlag)
               if (isfield(metaData.sensors.sensor_opus, 'sensor'))
                  if (isfield(metaData.sensors.sensor_opus.sensor, 'sn'))
                     metaData.sensors.sensor_opus.sensor.sn = regexprep(metaData.sensors.sensor_opus.sensor.sn, 'Opus_', '');
            ...
            if (isfield(metaData.sensors.sensor_opus, 'spectrum'))
               if (isfield(metaData.sensors.sensor_opus.spectrum, 'length'))
                  if (~isfield(o_metaStruct, 'META_AUX_NITRATE_SPECTRUM_LENGTH'))
                     o_metaStruct.META_AUX_NITRATE_SPECTRUM_LENGTH = metaData.sensors.sensor_opus.spectrum.length;
                  end
               end
               if (isfield(metaData.sensors.sensor_opus.spectrum, 'wavelengths'))
                  if (~isfield(o_metaStruct, 'META_AUX_NITRATE_SPECTRUM_WAVELENGTHS'))
                     o_metaStruct.META_AUX_NITRATE_SPECTRUM_WAVELENGTHS = metaData.sensors.sensor_opus.spectrum.wavelengths;
                  end
               end
            end
            ...
            if (isfield(metaData.sensors.sensor_opus, 'spectrometer'))
               if (isfield(metaData.sensors.sensor_opus.spectrometer, 'path'))
                  metaData.sensors.sensor_opus.spectrometer.path = regexprep(metaData.sensors.sensor_opus.spectrometer.path, ' mm', '');
                  % code à suivre en commentaire car on pensait en avoir besoin
                  % pour le calcul donc dans les CALIBRATION_COEFFICIENT on
                  % attend de voir si c'est juste une méta-donnée
                  %
                  % if (~isfield(o_metaStruct, 'CALIBRATION_COEFFICIENT'))
                  %    o_metaStruct.CALIBRATION_COEFFICIENT = [];
                  % end
                  % if (~isfield(o_metaStruct.CALIBRATION_COEFFICIENT, 'OPUS'))
                  %    o_metaStruct.CALIBRATION_COEFFICIENT.OPUS = [];
                  % end
                  % if (isfield(o_metaStruct.CALIBRATION_COEFFICIENT.OPUS, 'Pathlength_NITRATE'))
                  %    if (~strcmp(metaData.sensors.sensor_opus.spectrometer.path, o_metaStruct.CALIBRATION_COEFFICIENT.OPUS.Pathlength_NITRATE))
                  %       if (isempty(g_cogj_csvFileBddId) || (g_cogj_csvFileBddId == -1))
                  %          [g_cogj_csvFileBddId, g_cogj_csvFileBddPathName] = open_csv_file(a_outputCsvDirName, 'B', g_cogj_csvFileBddPathName);
                  %       end
                  %       fprintf('WARNING: BDD: Float #%d: Pathlength_NITRATE differ (database: ''%s'' xml:''%s'') - DB contents should be updated (see %s)\n', ...
                  %          a_floatNum, ...
                  %          o_metaStruct.CALIBRATION_COEFFICIENT.OPUS.Pathlength_NITRATE, ...
                  %          metaData.sensors.sensor_opus.spectrometer.path, ...
                  %          g_cogj_csvFileBddPathName);
                  %       fprintf(g_cogj_csvFileBddId, '%d;99999;99999;%s;Pathlength_NITRATE\n', ...
                  %          a_floatNum, ...
                  %          metaData.sensors.sensor_opus.spectrometer.path);
                  %    end
                  % else
                  %    o_metaStruct.CALIBRATION_COEFFICIENT.OPUS.Pathlength_NITRATE = metaData.sensors.sensor_opus.spectrometer.path;
                  % end
                  if (~isfield(o_metaStruct, 'META_AUX_OPUS_SPECTROMETER_PATH'))
                     o_metaStruct.META_AUX_OPUS_SPECTROMETER_PATH = metaData.sensors.sensor_opus.spectrometer.path;
                  end
               end
            end
   ...
   if (any(a_sensorListNum == 20))
      if (isfield(metaData.sensors, 'sensor_imu'))
         if (isfield(metaData.sensors.sensor_imu, 'sensor'))
            if (isfield(metaData.sensors.sensor_imu.sensor, 'sn'))
               if (~isempty(metaData.sensors.sensor_imu.sensor.sn))
                  idF = find(strcmp('AUX_INERTIAL_SENSOR', o_metaStruct.SENSOR));
                  if (~a_rtVersionFlag)
                     if (~isempty(idF))
                        if (~strcmp(metaData.sensors.sensor_imu.sensor.sn, o_metaStruct.SENSOR_SERIAL_NO{idF}))
                           if (isempty(g_cogj_csvFileBddId) || (g_cogj_csvFileBddId == -1))
                              [g_cogj_csvFileBddId, g_cogj_csvFileBddPathName] = open_csv_file(a_outputCsvDirName, 'B', g_cogj_csvFileBddPathName);
                           end
                           fprintf('WARNING: BDD: Float #%d: SENSOR_SERIAL_NO for ''%s'' differ (database: ''%s'' xml:''%s'') - DB contents should be updated (see %s)\n', ...
                              ...
                              'AUX_INERTIAL_SENSOR', ...
                              o_metaStruct.SENSOR_SERIAL_NO{idF}, ...
                              metaData.sensors.sensor_imu.sensor.sn, ...
                              g_cogj_csvFileBddPathName);
                           fprintf(g_cogj_csvFileBddId, '%d;411;901;%s;SENSOR_SERIAL_NO\n', ...
                              a_floatNum, ...
                              metaData.sensors.sensor_imu.sensor.sn);
