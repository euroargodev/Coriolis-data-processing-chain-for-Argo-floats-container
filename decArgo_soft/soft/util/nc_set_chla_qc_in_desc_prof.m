% ------------------------------------------------------------------------------
% Set CHLA_QC = '3' (when not already set to '4') in descending profiles and
% associated measurements of trajectory files.
%
% SYNTAX :
%   nc_set_chla_qc_in_desc_prof or nc_set_chla_qc_in_desc_prof(6900189, 7900118)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function nc_set_chla_qc_in_desc_prof(varargin)
...
% program version
global g_cosq_setChlaQcDescProf;
g_cosq_setChlaQcDescProf = '3.0';
...
try
   ...
   logFile = [DIR_LOG_FILE '/' 'nc_set_chla_qc_in_desc_prof' name '_' currentTime '.log'];
   diary(logFile);
   tic;
   ...
   for idFloat = 1:nbFloats
      ...
      if (exist(ncInputFileDir, 'dir') == 7)
         ...
         if (exist(ncInputFileDir, 'dir') == 7)
            ...
            ncInputFiles = dir([ncInputFileDir 'B*D.nc']);
            for idFile = 1:length(ncInputFiles)
               ...
               [modifiedFileFlag, tmpOutputFileName] = set_chla_qc_in_profile_file(monoProfInputFilePathName, DIR_TMP_FILES);
         ...
         if (~isempty(ncInputTraj))
            ...
            % process current file
            [modifiedFileFlag, tmpOutputFileName] = set_chla_qc_in_traj_file(trajInputFilePathName, DIR_TMP_FILES);
...
xmlFileName = [DIR_XML_FILE '/' 'nc_set_chla_qc_in_desc_prof' name '_' currentTime '.xml'];
...
% ------------------------------------------------------------------------------
% Set CHLA_QC = '3' (when not already set to '4') in descending profiles.
%
% SYNTAX :
%  [o_modifiedFileFlag, o_profOutputFileName] = ...
%    set_chla_qc_in_profile_file(a_profInputFileName, a_tmpDirName)
%
% INPUT PARAMETERS :
   ...
   try
      ...
      for idParamList = 1:length(paramList)
         ...
         if (~isempty(modifiedProfId))
            ...
            histoInstitution = 'IF';
            histoSoftware = 'COSQ';
            histoSoftwareRelease = g_cosq_setChlaQcDescProf;
...
% ------------------------------------------------------------------------------
% Set CHLA_QC = '3' (when not already set to '4') in associated measurements of
% trajectory file.
%
% SYNTAX :
%  [o_modifiedFileFlag, o_trajOutputFileName] = ...
%    set_chla_qc_in_traj_file(a_trajInputFileName, a_tmpDirName)
%
% INPUT PARAMETERS :
         ...
         if (~isempty(paramName))
            if (ismember(paramName, paramList))
               ...
               idToSet = find(((dataQc == g_decArgo_qcStrNoQc) | (dataQc == g_decArgo_qcStrGood)) & ...
                  ((measurementCode == g_MC_DescProf) | (measurementCode == g_MC_DescProfDeepestBin)));
               if (~isempty(idToSet))
                  % updated <PARAM>_QC
...
newChild.appendChild(docNode.createTextNode('Argo Coriolis set CHLA_QC in descending profile file tool (nc_set_chla_qc_in_desc_prof)'));
docRootNode.appendChild(newChild);

newChild = docNode.createElement('tool_version');
newChild.appendChild(docNode.createTextNode(g_cosq_setChlaQcDescProf));
docRootNode.appendChild(newChild);
