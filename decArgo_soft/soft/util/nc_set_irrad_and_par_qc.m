% ------------------------------------------------------------------------------
% Initialize DOWN_IRRADIANCExxx_QC and DOWNWELLING_PAR_QC to '2' in profile and
% trajectory files.
%
% SYNTAX :
%   nc_set_irrad_and_par_qc or nc_set_irrad_and_par_qc(6900189, 7900118)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function nc_set_irrad_and_par_qc(varargin)
...
% DIR_INPUT_NC_FILES = 'F:\snapshot-202405\coriolis\';
...
% program version
global g_cosq_setIrradAndParQc;
g_cosq_setIrradAndParQc = '4.0';
...
try
   ...
   logFile = [DIR_LOG_FILE '/' 'nc_set_irrad_and_par_qc' name '_' currentTime '.log'];
   diary(logFile);
   tic;
   ...
   for idFloat = 1:nbFloats
      ...
      if (exist(ncInputFileDir, 'dir') == 7)
         ...
         if (updateFiles == 0)
            ...
            ncOutputFileDir = [DIR_INPUT_NC_FILES '/' num2str(floatNum) '/'];
         end
         ...
         if (exist(ncInputFileDir, 'dir') == 7)
            ...
            % create output directory
            ncOutputProfFileDir = [ncOutputFileDir '/profiles/'];
            if (updateFiles == 0)
               if ~(exist(ncOutputProfFileDir, 'dir') == 7)
                  if (g_cosq_doItFlag == 1)
                     mkdir(ncOutputProfFileDir);
                  end
               end
            end

            ncInputFiles = dir([ncInputFileDir 'B*.nc']);
            for idFile = 1:length(ncInputFiles)
               ...
               monoProfOutputFilePathName = [ncOutputProfFileDir '/' monoProfInputFileName];

               fprintf('%s\n', monoProfInputFileName);

               % process current file
               [modifiedFileFlag, tmpOutputFileName] = set_irrad_and_par_qc_in_profile_file(monoProfInputFilePathName, DIR_TMP_FILES);

               if (modifiedFileFlag == 1)

                  if (g_cosq_doItFlag == 1)
                     move_file(tmpOutputFileName, monoProfOutputFilePathName);
                  end

                  % store the information for the XML report
                  g_cosq_reportData.float = [g_cosq_reportData.float g_cosq_floatNum];
                  g_cosq_reportData.monoProfFile = [g_cosq_reportData.monoProfFile {monoProfOutputFilePathName}];
               end
            end
         else
         ...
         ncInputFileDir = [DIR_INPUT_NC_FILES '/' num2str(floatNum) '/'];
         ncInputTraj = dir([ncInputFileDir floatNumStr '_*traj.nc']);
         if (~isempty(ncInputTraj))
            trajInputFileName = ncInputTraj(1).name;

            trajInputFilePathName = [ncInputFileDir '/' trajInputFileName];
            trajfOutputFilePathName = [ncOutputFileDir '/' trajInputFileName];

            fprintf('%s\n', trajInputFileName);

            % process current file
            [modifiedFileFlag, tmpOutputFileName] = set_irrad_and_par_qc_in_traj_file(trajInputFilePathName, DIR_TMP_FILES);

            if (modifiedFileFlag == 1)

               if (g_cosq_doItFlag == 1)
                  move_file(tmpOutputFileName, trajfOutputFilePathName);
               end

               % store the information for the XML report
               g_cosq_reportData.float = [g_cosq_reportData.float g_cosq_floatNum];
               g_cosq_reportData.trajFile = [g_cosq_reportData.trajFile {trajfOutputFilePathName}];
            end
         end
      else
         fprintf('WARNING: Directory not found: %s\n', ncInputFileDir);
...
xmlFileName = [DIR_XML_FILE '/' 'nc_set_irrad_and_par_qc' name '_' currentTime '.xml'];
...
% ------------------------------------------------------------------------------
% Initialize DOWN_IRRADIANCExxx_QC and DOWNWELLING_PAR_QC to '2' in profile
% file.
%
% SYNTAX :
%  [o_modifiedFileFlag, o_profOutputFileName] = ...
%    set_irrad_and_par_qc_in_profile_file(a_profInputFileName, a_tmpDirName)
%
% INPUT PARAMETERS :
...
for idProf = 1:nProf
   for idParam = 1:nParam
      ...
      if (~isempty(paramName))
         if (ismember(paramName, paramList))
            profIdList = [profIdList idProf];
         end
      end
   end
end
profIdList = unique(profIdList);
...
if (~isempty(profIdList))
   ...
   try
      ...
      % process each param separately for HISTORY_PARAMETER report
      for idParamList = 1:length(paramList)
         oneParamFromList = paramList{idParamList};
         ...
         for idProf = profIdList
            for idParam = 1:nParam
               paramName = deblank(stationParameters(:, idParam, idProf)');
               if (~isempty(paramName))
                  if (strcmp(paramName, oneParamFromList))

                     paramQcName = [paramName '_QC'];
                     dataQc = netcdf.getVar(fCdf, netcdf.inqVarID(fCdf, paramQcName));
                     % some of these parameters have a global range test defined yet => QC set to '1' or '4'
                     % others don't have any RTQC test defined yet => QC set to '0'
                     idToSet = find((dataQc(:, idProf) == g_decArgo_qcStrNoQc) | (dataQc(:, idProf) == g_decArgo_qcStrGood));
                     if (~isempty(idToSet))
                        dataQc(idToSet, idProf) = g_decArgo_qcStrProbablyGood;
                        ...
                        if (parameterDataMode(idProf, idParam) == 'R')
                           ...
                           profParamQc = netcdf.getVar(fCdf, netcdf.inqVarID(fCdf, profParamQcName));
                           newProfParamQc = compute_profile_quality_flag(dataQc(:, idProf));
                           profParamQc(idProf) = newProfParamQc;
                           netcdf.putVar(fCdf, netcdf.inqVarID(fCdf, profParamQcName), profParamQc);
                        end
                     end
                  end
               end
         ...
         if (~isempty(modifiedProfId))
            ...
            histoInstitution = 'IF';
            histoSoftware = 'COSQ';
            histoSoftwareRelease = g_cosq_setIrradAndParQc;
            histoParameter = oneParamFromList;
...
% ------------------------------------------------------------------------------
% Initialize DOWN_IRRADIANCExxx_QC and DOWNWELLING_PAR_QC to '2' in trajectory
% file.
%
% SYNTAX :
%  [o_modifiedFileFlag, o_trajOutputFileName] = ...
%    set_irrad_and_par_qc_in_traj_file(a_trajInputFileName, a_tmpDirName)
%
% INPUT PARAMETERS :
...
if (toBeCheckFlag == 1)
   ...
   % make a copy of the traj file to be updated
      ...
      for idParam = 1:nParam
         ...
         if (~isempty(paramName))
            if (ismember(paramName, paramList))
               ...
               % some of these parameters have a global range test defined yet => QC set to '1' or '4'
               % others don't have any RTQC test defined yet => QC set to '0'
               idToSet = find((dataQc == g_decArgo_qcStrNoQc) | (dataQc == g_decArgo_qcStrGood));
               if (~isempty(idToSet))
                  % updated <PARAM>_QC
                  dataQc(idToSet) = g_decArgo_qcStrProbablyGood;
                  netcdf.putVar(fCdf, netcdf.inqVarID(fCdf, [paramName '_QC']), dataQc);
                  modifiedParamList{end+1} = paramName;
               end
            end
         end
      end
   ...
   if (~isempty(modifiedParamList))

      if (g_cosq_doItFlag == 1)

         % N_HISTORY dimension is not UNLIMITED, we should update this dimension
         ok = update_n_history_dim_in_traj_file(tmpProfOutputFileName, length(modifiedParamList));
         ...
         try
            ...
            % update history information
            historyInstitution = netcdf.getVar(fCdf, netcdf.inqVarID(fCdf, 'HISTORY_INSTITUTION'));
            [~, nHistoryFinal] = size(historyInstitution);
            for idParam = 1:length(modifiedParamList)

               nHistory = nHistoryFinal - length(modifiedParamList) + idParam;

               histoInstitution = 'IF';
               histoSoftware = 'COSQ';
               histoSoftwareRelease = g_cosq_setIrradAndParQc;
               histoParameter = modifiedParamList{idParam};
...
newChild.appendChild(docNode.createTextNode('Argo Coriolis set DOWN_IRRADIANCE*_QC and DOWNWELLING_PAR in profile file tool (nc_set_irrad_and_par_qc)'));
docRootNode.appendChild(newChild);

newChild = docNode.createElement('tool_version');
newChild.appendChild(docNode.createTextNode(g_cosq_setIrradAndParQc));
docRootNode.appendChild(newChild);
