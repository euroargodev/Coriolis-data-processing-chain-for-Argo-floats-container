...
%    process_profile_cts5_usea_opus_black_v2(a_opusBlackV2Data, a_timeData, a_gpsData, a_opusLightNValues)
%
% INPUT PARAMETERS :
%   a_opusBlackV2Data  : CTS5-USEA OPUS-BLACK V2 data
%   a_timeData         : decoded time data
%   a_gpsData          : GPS data
%   a_opusLightNValues : N_VALUES dimension of associated OPUS-LIGHT data
%
% OUTPUT PARAMETERS :
...
for idP = 1:length(a_opusBlackV2Data)
   ...
   if (~isempty(data))
      ...
      profStruct.paramNumberOfSubLevels = a_opusLightNValues;

      profStruct.data = [ ...
         data(:, 2) ... % PRES
         ones(size(data, 1), 1)*5 ... % SPECTRUM_TYPE_NITRATE
         data(:, 3:6) ... % AVERAGING_NITRATE, FLASH_COUNT_NITRATE, TEMP_SPECTROPHOTOMETER_NITRATE, LAMP_REFERENCE_1
         ones(size(data, 1), a_opusLightNValues)*paramUvIntensityNitrate.fillValue ... % UV_INTENSITY_NITRATE
         data(:, 7:8) ... % UV_INTENSITY_DARK_NITRATE_AVG, UV_INTENSITY_DARK_NITRATE_SD
         ];
