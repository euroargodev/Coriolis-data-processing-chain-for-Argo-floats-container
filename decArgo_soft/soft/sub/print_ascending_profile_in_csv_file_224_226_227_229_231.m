...
%    a_ascProfPres, a_ascProfTemp, a_ascProfSal, a_ascProfSalCor, ascProfTempCndc, ...
...
% ------------------------------------------------------------------------------
function print_ascending_profile_in_csv_file_224_226_227_229_231( ...
   a_ascProfDate, a_ascProfDateAdj, ...
   a_ascProfPres, a_ascProfTemp, a_ascProfSal, a_ascProfSalCor, ascProfTempCndc, ...
   a_ascProfC1PhaseDoxy, a_ascProfC2PhaseDoxy, a_ascProfTempDoxy, a_ascProfDoxy)
...
if (~isempty(a_ascProfPres))
   ...
   if (isempty(a_ascProfC1PhaseDoxy))
      if (~isempty(a_ascProfSal) && isempty(a_ascProfSalCor) && isempty(ascProfTempCndc))
      ...
      elseif (~isempty(a_ascProfSal) && isempty(a_ascProfSalCor) && ~isempty(ascProfTempCndc))
      ...
      elseif (~isempty(a_ascProfSal) && ~isempty(a_ascProfSalCor) && isempty(ascProfTempCndc))
         ...
         fprintf(g_decArgo_outputCsvFileId, '%d; %d; AscProf; Description; Float time; UTC time; PRES (dbar); TEMP (degC); PSAL (PSU); PSAL_DynamicCorrection (PSU)\n', ...
            g_decArgo_floatNum, g_decArgo_cycleNum);
         ...
         for idMes = 1:length(a_ascProfPres)
            ...
            fprintf(g_decArgo_outputCsvFileId, '%d; %d; AscProf; Asc. profile meas. #%d; %s; %s; %.1f; %.3f; %.3f; %.3f\n', ...
               g_decArgo_floatNum, g_decArgo_cycleNum, ...
               idMes, mesDateStr, mesDateAdjStr, ...
               a_ascProfPres(idMes), a_ascProfTemp(idMes), a_ascProfSal(idMes), a_ascProfSalCor(idMes));
         end

      elseif (isempty(a_ascProfSal) && ~isempty(a_ascProfSalCor) && isempty(ascProfTempCndc))

         fprintf(g_decArgo_outputCsvFileId, '%d; %d; AscProf; Description; Float time; UTC time; PRES (dbar); TEMP (degC); PSAL_DynamicCorrection (PSU)\n', ...
            g_decArgo_floatNum, g_decArgo_cycleNum);
         ...
         for idMes = 1:length(a_ascProfPres)
            ...
            fprintf(g_decArgo_outputCsvFileId, '%d; %d; AscProf; Asc. profile meas. #%d; %s; %s; %.1f; %.3f; %.3f\n', ...
               g_decArgo_floatNum, g_decArgo_cycleNum, ...
               idMes, mesDateStr, mesDateAdjStr, ...
               a_ascProfPres(idMes), a_ascProfTemp(idMes), a_ascProfSalCor(idMes));
         end
