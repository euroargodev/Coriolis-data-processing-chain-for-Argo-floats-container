...
%
% SYNTAX :
% [o_profDo, o_profCtdPtsh, o_profCtdCpH, o_profFlbb, o_profFlbbCd, ...
%   o_profRamses, o_profRafos] = ...
%   compute_derived_parameters_apx_apf11_ir( ...
%   a_profCtdPts, a_profCtdCp, a_profDo, ...
%   a_profCtdPtsh, a_profCtdCpH, a_profFlbb, a_profFlbbCd, a_profRamses, ...
%   a_profRafos, ...
...
% ------------------------------------------------------------------------------
function [o_profDo, o_profCtdPtsh, o_profCtdCpH, o_profFlbb, o_profFlbbCd, ...
   o_profRamses, o_profRafos] = ...
   compute_derived_parameters_apx_apf11_ir( ...
   a_profCtdPts, a_profCtdCp, a_profDo, ...
   a_profCtdPtsh, a_profCtdCpH, a_profFlbb, a_profFlbbCd, a_profRamses, ...
...
switch (a_decoderId)
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   case {1121, 1122, 1123, 1124, 1125, 1126, 1127, 1128, 1129, 1130, 1131, 1132, 1322, 1323}
         ...
         if (~isempty(idPres) && ...
               ...
               if (length(idPark) > 1)
                  ...
                  if (~isempty(ctdData))
                     ...
                     if (~isempty(ctdLinkData))
                        ...
                        % compute DOXY
                        doxyValues = compute_DOXY_1121_to_32_1322_1323( ...
            ...
            if (~isempty(a_cycleTimeData.ascentEndDate))
               ...
               if (~isempty(idSurf))
                  ...
                  % compute PPOX_DOXY
                  ppoxDoxyValues = compute_PPOX_DOXY_1121_to_32_1322_1323( ...
      ...
      if ((~isempty(o_profCtdPtsh)) || (~isempty(o_profCtdCpH)))
         ...
         if (~isempty(o_profCtdPtsh))
            ...
            if (~isempty(idPres) && ~isempty(idTemp) && ~isempty(idPsal) && ...
               ...
               [phInSituFreeValues, phInSituTotalValues] = compute_PH_apex( ...
      ...
      if (~isempty(o_profFlbb))
         ...
         if (~isempty(idFluorescenceChla) && ~isempty(idChla) && ~isempty(idChlaFluo))
            % compute CHLA
            chlaValues = compute_CHLA_1xx_and_1121_to_1132_1322_1323( ...
               ...
               if (length(idPark) > 1)
                  ...
                  if (~isempty(ctdData))
                     ...
                     if (~isempty(ctdLinkData))
                        ...
                        % compute BBP700
                        bbp700Values = compute_BBP700_others_dec_id( ...
      ...
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      % DOWN_IRRADIANCE_SPECTRUM & DOWN_IRRADIANCE_SPECTRUM_WAVELENGTHS
      % (RAMSES_ACC derived parameter)

      if (~isempty(o_profRamses))

         paramDownIrrSpectrum = get_netcdf_param_attributes('DOWN_IRRADIANCE_SPECTRUM');
         paramDownIrrSpectrumWavelength = get_netcdf_param_attributes('DOWN_IRRADIANCE_SPECTRUM_WAVELENGTHS');

         paramDownIrrIntegTime = get_netcdf_param_attributes('RADIOMETER_DOWN_IRR_INTEGRATION_TIME');
         paramRawDownIrr = get_netcdf_param_attributes('RAW_DOWNWELLING_IRRADIANCE');

         % add DOWN_IRRADIANCE_SPECTRUM and DOWN_IRRADIANCE_SPECTRUM_WAVELENGTHS
         % to the RAMSES profile
         o_profRamses.paramList = [o_profRamses.paramList paramDownIrrSpectrum paramDownIrrSpectrumWavelength];
         o_profRamses.data = [o_profRamses.data ...
            ones(size(o_profRamses.data, 1), 255)*paramDownIrrSpectrum.fillValue ...
            ones(size(o_profRamses.data, 1), 255)*paramDownIrrSpectrumWavelength.fillValue];
         if (~isempty(o_profRamses.dataAdj))
            o_profRamses.paramDataMode = [o_profRamses.paramDataMode '  '];
            o_profRamses.dataAdj = [o_profRamses.dataAdj ...
               ones(size(o_profRamses.dataAdj, 1), 255)*paramDownIrrSpectrum.fillValue ...
               ones(size(o_profRamses.dataAdj, 1), 255)*paramDownIrrSpectrumWavelength.fillValue];
         end

         % DOWN_IRRADIANCE_SPECTRUM & DOWN_IRRADIANCE_SPECTRUM_WAVELENGTHS
         idDownIrrIntegTime = find(strcmp({o_profRamses.paramList.name}, 'RADIOMETER_DOWN_IRR_INTEGRATION_TIME') == 1, 1);
         idRawDownIrr = find(strcmp({o_profRamses.paramList.name}, 'RAW_DOWNWELLING_IRRADIANCE') == 1, 1);

         if (~isempty(idDownIrrIntegTime) && ~isempty(idRawDownIrr))
            % compute DOWN_IRRADIANCE_SPECTRUM & DOWN_IRRADIANCE_SPECTRUM_WAVELENGTHS
            [downIrrSpectrum, downIrrSpectrumWavelengths] = ...
               compute_DOWN_IRRADIANCE_SPECTRUM_apx_apf11( ...
               o_profRamses.data(:, idDownIrrIntegTime), ...
               o_profRamses.data(:, idRawDownIrr:idRawDownIrr+o_profRamses.paramNumberOfSubLevels-1), ...
               paramDownIrrIntegTime.fillValue, ...
               paramRawDownIrr.fillValue, ...
               paramDownIrrSpectrum.fillValue, ...
               paramDownIrrSpectrumWavelength.fillValue, ...
               o_profRamses);

            colStart = length(o_profRamses.paramList)-2 + o_profRamses.paramNumberOfSubLevels;
            colStop = colStart + 254;
            o_profRamses.data(:, colStart:colStop) = downIrrSpectrum;
            colStart = colStop + 1;
            colStop = colStart + 254;
            o_profRamses.data(:, colStart:colStop) = downIrrSpectrumWavelengths;
         end

         o_profRamses.paramNumberWithSubLevels = [o_profRamses.paramNumberWithSubLevels length(o_profRamses.paramList)-1 length(o_profRamses.paramList)];
         o_profRamses.paramNumberOfSubLevels = [o_profRamses.paramNumberOfSubLevels 255 255];
      end
