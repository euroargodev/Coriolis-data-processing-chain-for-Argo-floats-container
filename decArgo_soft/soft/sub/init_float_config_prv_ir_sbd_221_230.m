...
% ------------------------------------------------------------------------------
function init_float_config_prv_ir_sbd_221_230(a_launchDate, a_decoderId)
         ...
         if (a_decoderId == 230)
            ...
            for id = 0:3
               ...
               if (isfield(calibData, fieldName))
                  ...
                  fprintf('ERROR: Float #%d: inconsistent CALIBRATION_COEFFICIENT information for OPTODE (AANDERAA) sensor\n', g_decArgo_floatNum);
                  return
               end
            end
            for id = 0:6
            ...
            coef = 'ABCDEF';
            for id = 1:length(coef)
               fieldName = ['ArodFtOptodeTemp' coef(id)];
               if (isfield(calibData, fieldName))
                  tabDoxyCoef(3, id) = calibData.(fieldName);
               else
                  fprintf('ERROR: Float #%d: inconsistent CALIBRATION_COEFFICIENT information for OPTODE (AROD_FT) sensor\n', g_decArgo_floatNum);
                  return
               end
            end
            for id = 0:2
               fieldName = ['ArodFtOptodeC' num2str(id)];
               if (isfield(calibData, fieldName))
            ...
            for id = 0:4
               fieldName = ['ArodFtOptodeD' num2str(id)];
               if (isfield(calibData, fieldName))
            ...
            fieldName = 'ArodFtOptodeE0';
            if (isfield(calibData, fieldName))
               tabDoxyCoef(6, 1) = calibData.(fieldName);
            else
               fprintf('ERROR: Float #%d: inconsistent CALIBRATION_COEFFICIENT information for OPTODE (AROD_FT) sensor\n', g_decArgo_floatNum);
               return
            end
            fieldName = 'ArodFtOptodeCP';
            if (isfield(calibData, fieldName))
               tabDoxyCoef(7, 1) = calibData.(fieldName);
            else
               fprintf('ERROR: Float #%d: inconsistent CALIBRATION_COEFFICIENT information for OPTODE (AROD_FT) sensor\n', g_decArgo_floatNum);
               return
            end
