...
% SYNTAX :
%  [o_cycleTimeData] = ...
%    compute_prv_dates_230(a_tabTech1, a_tabTech2, a_deepCycle, ...
%    a_refDay, a_cycleNum)
%
% INPUT PARAMETERS :
...
elseif (length(idF2) == 1)
   ...
   if (a_tabTech2(id, 20) > 0)
      
      if (~isempty(cycleStartDate))
         if (a_tabTech2(id, 22) == 255) % see 6902893 ##52, 60:68, 73, 74
            firstGroundingDate = fix(cycleStartDate) + a_tabTech2(id, 23)/1440;
         else
            firstGroundingDate = fix(cycleStartDate) + a_tabTech2(id, 22) + a_tabTech2(id, 23)/1440;
         end
      end
      firstGroundingPres = a_tabTech2(id, 21);
   end
   
   if (a_tabTech2(id, 20) > 1)
      
      if (~isempty(cycleStartDate))
         if (a_tabTech2(id, 27) == 255) % see 6902893 ##52, 60:68, 73, 74
            secondGroundingDate = fix(cycleStartDate) + a_tabTech2(id, 28)/1440;
         else
            secondGroundingDate = fix(cycleStartDate) + a_tabTech2(id, 27) + a_tabTech2(id, 28)/1440;
         end
      end
      secondGroundingPres = a_tabTech2(id, 26);
   end

   if (a_tabTech2(id, 31) > 0)
      
      % manage possible roll over of first emergency ascent day
      firstEmergencyAscentDay = a_tabTech2(id, 35);
      if (~isempty(cycleStartDate))
         while ((a_refDay + firstEmergencyAscentDay + a_tabTech2(id, 32)/1440) < cycleStartDate)
            firstEmergencyAscentDay = firstEmergencyAscentDay + 256;
         end
      end
      
      firstEmergencyAscentDate = a_refDay + firstEmergencyAscentDay + a_tabTech2(id, 32)/1440;
      firstEmergencyAscentPres = a_tabTech2(id, 33);
   end   
end
