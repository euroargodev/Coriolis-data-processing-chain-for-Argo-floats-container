...
switch (a_decoderId)
   ...
   case {121, 122, 123, 124, 125, 126, 127, 128, 129, 130, 131, 132, 133, 134, 135, 136, 137, 138, 139, 140, 141}
      ...
      if (ismember(a_decoderId, g_decArgo_decoderIdListNkeCts5Usea))
         metaAuxLabelList = [ ...
            ...
            {'META_AUX_NITRATE_SPECTRUM_LENGTH'} ...
            {'META_AUX_NITRATE_SPECTRUM_WAVELENGTHS'} ...
            {'META_AUX_OPUS_SENSOR_LAMP_SERIAL_NO'} ...
            {'META_AUX_OPUS_SPECTROMETER_PATH'} ...
      ...
      % put CONFIG_BitMaskMonthsIceDetectionActive_NUMBER and
      % CONFIG_BitMaskMonthsIceEvasionActive_NUMBE months in reverse order
      if (ismember(a_decoderId, g_decArgo_decoderIdListNkeCts5Osean))
         idF = find(inputDynamicConfigId == 205);
         if (~isempty(idF))
            for idC = 1:size(inputDynamicConfigValue, 2)
               if (~isnan(inputDynamicConfigValue(idF, idC)))
                  inputDynamicConfigValue(idF, idC) = bin2dec(fliplr(dec2bin(inputDynamicConfigValue(idF, idC), 12)));
               end
            end
         end
         idF = find(inputDynamicConfigId == 222);
         if (~isempty(idF))
            for idC = 1:size(inputDynamicConfigValue, 2)
               if (~isnan(inputDynamicConfigValue(idF, idC)))
                  inputDynamicConfigValue(idF, idC) = bin2dec(fliplr(dec2bin(inputDynamicConfigValue(idF, idC), 12)));
   ...
   case {221, 228, 229, 230}
      ...
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   case {222, 223, 224, 225, 226, 227, 231, 232}

      if (~isempty(a_structConfig))
   ...
   case {401, 402, 403}
      ...
      % concat the input configuration information
      configName = a_structConfig.DYNAMIC_NC.NAMES;
      configId = a_structConfig.DYNAMIC_NC.IDS;
      configValue = a_structConfig.DYNAMIC_NC.VALUES;
...
try
   ...
   institution = '';
   if (isfield(metaData, 'DATA_CENTRE'))
      institution = get_institution_from_data_centre(metaData.DATA_CENTRE, 1);
   end
   ...
   programNameVarId = netcdf.defVar(fCdf, 'PROGRAM_NAME', 'NC_CHAR', string64DimId);
   netcdf.putAtt(fCdf, programNameVarId, 'long_name', 'Name of the program');
   netcdf.putAtt(fCdf, programNameVarId, '_FillValue', ' ');
   floatNcVarId = [floatNcVarId; programNameVarId];
   floatNcVarName{end+1} = 'PROGRAM_NAME';
   ...
   deploymentPlatformVarId = netcdf.defVar(fCdf, 'DEPLOYMENT_PLATFORM', 'NC_CHAR', string128DimId);
   ...
   for idField = 1:length(metaFieldNames)
      ...
      if (~isempty(inputElt))
         ...
         elseif (isa(inputElt, 'struct'))
            ...
            % sort fields according to their number
            if (length(fieldNames) > 1)
               numList = nan(length(fieldNames), 1);
               for id = 1:length(fieldNames)
                  idF = strfind(fieldNames{id}, '_');
                  numList(id) = str2double(fieldNames{id}(idF(end)+1:end));
               end
               [~, sortId] = sort(numList);
            else
               sortId = 1;
            ...
            for id = 1:length(fieldNames)
               valueStr = inputElt.(fieldNames{sortId(id)});
               if (~isempty(valueStr))
                  [varName, xType, dimIds, nAtts] = netcdf.inqVar(fCdf, floatNcVarId(idMeta));
