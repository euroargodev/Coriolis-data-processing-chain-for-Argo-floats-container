...
%
% SYNTAX :
%  [o_opusLightData] = decode_apmt_opus_light_v2(a_data, a_lastByteNum, a_inputFilePathName)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
% RELEASES :
%   05/15/2025 - RNU - creation
% ------------------------------------------------------------------------------
function [o_opusLightData] = decode_apmt_opus_light_v2(a_data, a_lastByteNum, a_inputFilePathName)
...
while (currentByte <= lastByteNum)
   ...
   if ((currentPhaseNum ~= -1) && (currentTreatNum ~= -1))
      ...
      dataAll = [data(1:7) nbF nan(1, 255) nbB nan(1, 255)];
      dataAll(9:9+length(dataF)-1) = dataF;
      dataAll(265:265+length(dataB)-1) = dataB;
