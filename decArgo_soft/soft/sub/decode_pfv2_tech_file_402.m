...
% ------------------------------------------------------------------------------
% RELEASES :
%   04/22/2025 - RNU - creation
% ------------------------------------------------------------------------------
function [o_techData] = decode_pfv2_tech_file_402(a_fileName)
...
tabEvtBis = []; % for CSV output only (but not for the current mission or cycle)
tabEvtNum = []; % to check that all events have been checked once by the operator
tabTech = []; % for TECH and TECH_AUX files
tabTechTime = []; % for TECH_TIME and TECH_AUX_TIME files
tabTechBis = []; % for TECH and TECH_AUX files (but not for the current mission or cycle)
tabTraj = []; % for TRAJ
   ...
   switch (evtNum)
      ...
      case 1000
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            evt = get_pfv2_tech_event_data_init_struct(100001);
            evt.class = 'PHASE (BUOYANCY REDUCTION START)';
            evt.label = 'External pressure (dbar)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            data = data/10;
      ...
      case 1002
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            evt = get_pfv2_tech_event_data_init_struct(100201);
            evt.class = 'PHASE (BUOYANCY INVERSION START)';
            evt.label = 'External pressure (dbar)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            data = data/10;
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'PHASE (BUOYANCY INVERSION END): External pressure (dbar)';
            dataStruct.techId = 100201;
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            data = data/10;
            dataStruct.value = sprintf('%.1f', data);
            tabTech = [tabTech dataStruct];
      ...
      case 1004
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            evt = get_pfv2_tech_event_data_init_struct(100401);
            evt.class = 'PHASE (DESCENT TO PARKING START)';
            evt.label = 'External pressure (dbar)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            data = data/10;
            ...
            dataStruct = get_pfv2_traj_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'PHASE (DESCENT TO PARKING START): External pressure (dbar)';
            dataStruct.techId = 100401;
            [pres, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            pres = pres/10;
            dataStruct.measCode = g_MC_DST;
            dataStruct.pres = pres;
      ...
      case 1006
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            evt = get_pfv2_tech_event_data_init_struct(100601);
            evt.class = 'PHASE (DESCENT TO PROFILE START)';
            evt.label = 'External pressure (dbar)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            data = data/10;
            ...
            dataStruct = get_pfv2_traj_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'PHASE (DESCENT TO PROFILE START): External pressure (dbar)';
            dataStruct.techId = 100601;
            [pres, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            pres = pres/10;
            dataStruct.measCode = g_MC_PET;
            dataStruct.pres = pres;
      ...
      case 1008
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            evt = get_pfv2_tech_event_data_init_struct(100801);
            evt.class = 'PHASE (PARK START TIME)';
            evt.label = 'External pressure (dbar)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            data = data/10;
            ...
            dataStruct = get_pfv2_traj_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'PHASE (PARK START TIME): External pressure (dbar)';
            dataStruct.techId = 100801;
            [pres, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            pres = pres/10;
            dataStruct.measCode = g_MC_PST;
            dataStruct.pres = pres;
      ...
      case 1010
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            evt = get_pfv2_tech_event_data_init_struct(101001);
            evt.class = 'PHASE (DEEP PARK START TIME)';
            evt.label = 'External pressure (dbar)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            data = data/10;
            ...
            dataStruct = get_pfv2_traj_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'PHASE (DEEP PARK START TIME): External pressure (dbar)';
            dataStruct.techId = 101001;
            [pres, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            pres = pres/10;
            dataStruct.measCode = g_MC_DPST;
            dataStruct.pres = pres;
      ...
      case 1012
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            evt = get_pfv2_tech_event_data_init_struct(101201);
            evt.class = 'PHASE (ASCENT START TIME)';
            evt.label = 'External pressure (dbar)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            data = data/10;
            ...
            dataStruct = get_pfv2_traj_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'PHASE (ASCENT START TIME): External pressure (dbar)';
            dataStruct.techId = 101201;
            [pres, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            pres = pres/10;
            dataStruct.measCode = g_MC_AST;
            dataStruct.pres = pres;
      ...
      case 1014
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            evt = get_pfv2_tech_event_data_init_struct(101401);
            evt.class = 'PHASE (EMERGENCE START TIME)';
            evt.label = 'External pressure (dbar)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            data = data/10;
            ...
            dataStruct = get_pfv2_traj_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'PHASE (EMERGENCE START TIME): External pressure (dbar)';
            dataStruct.techId = 101401;
            [pres, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            pres = pres/10;
            dataStruct.measCode = g_MC_AET;
            dataStruct.pres = pres;
      ...
      case 1015
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            evt = get_pfv2_tech_event_data_init_struct(101501);
            evt.class = 'PHASE (IN AIR START TIME)';
            evt.label = 'External pressure (dbar)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            data = data/10;
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'PHASE (IN AIR START TIME): External pressure (dbar)';
            dataStruct.techId = 101501;
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            data = data/10;
            dataStruct.value = sprintf('%.1f', data);
            tabTech = [tabTech dataStruct];
      ...
      case 1016
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            evt = get_pfv2_tech_event_data_init_struct(101601);
            evt.class = 'PHASE (TRANSMISSION START TIME)';
            evt.label = 'External pressure (dbar)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            data = data/10;
      ...
      case 2004
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            evt.class = 'ISA DETECTION FILTERING';
            evt.label = 'Number max of detection before applying the breakup period';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 1);
            evt.valueStr = num2str(data);
            tabEvt = [tabEvt evt];
         else
      ...
      case 2006
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            evt = get_pfv2_tech_event_data_init_struct(200602);
            evt.class = 'ISA MEASURES';
            ...
            evt = get_pfv2_tech_event_data_init_struct(200603);
            evt.class = 'ISA MEASURES';
            ...
            dataJuld = epoch2000_2_julian(dataEpoch);
            [pres, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            [temp, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            [psal, curByte] = get_bytes_pfv2(dataIn, curByte, 2);

            dataStruct = get_pfv2_traj_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'ISA MEASURES';
            dataStruct.techId = 200600;
            dataStruct.measCode = g_MC_AscProf;
            dataStruct.julD = dataJuld;
            dataStruct.pres = pres;
      ...
      case 2010
         if (~isempty(g_decArgo_outputCsvFileId))
            evt = get_pfv2_tech_event_data_init_struct(201000);
            evt.class = 'ASCENT PERIGEE';
            evt.label = 'Time';
            [dataEpoch, curByte] = get_bytes_pfv2(dataIn, curByte, 4);
            dataJuld = epoch2000_2_julian(dataEpoch);
            evt.valueStr = julian_2_gregorian_dec_argo(dataJuld);
            ...
            evt = get_pfv2_tech_event_data_init_struct(201001);
            evt.class = 'ASCENT PERIGEE';
            evt.label = 'External pressure (dbar)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            data = data/10;
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'ASCENT PERIGEE: Time';
            dataStruct.techId = 201000;
            [dataEpoch, curByte] = get_bytes_pfv2(dataIn, curByte, 4);
            dataJuld = epoch2000_2_julian(dataEpoch);
            dataStruct.julD = dataJuld;
            dataStruct.value = datestr(dataJuld + g_decArgo_janFirst1950InMatlab, 'yyyymmddHHMMSS');
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'ASCENT PERIGEE: External pressure (dbar)';
            dataStruct.techId = 201001;
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            data = data/10;
            dataStruct.value = sprintf('%.1f', data);
            tabTech = [tabTech dataStruct];
      ...
      case 2011
         if (~isempty(g_decArgo_outputCsvFileId))
            evt = get_pfv2_tech_event_data_init_struct(201100);
            evt.class = 'ICE ALGORITHM ACTIVATED';
            evt.label = 'Time';
            [dataEpoch, curByte] = get_bytes_pfv2(dataIn, curByte, 4);
            dataJuld = epoch2000_2_julian(dataEpoch);
            evt.valueStr = julian_2_gregorian_dec_argo(dataJuld);
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'ICE ALGORITHM ACTIVATED: Time';
            dataStruct.techId = 201100;
            [dataEpoch, curByte] = get_bytes_pfv2(dataIn, curByte, 4);
            dataJuld = epoch2000_2_julian(dataEpoch);
            dataStruct.julD = dataJuld;
            dataStruct.value = datestr(dataJuld + g_decArgo_janFirst1950InMatlab, 'yyyymmddHHMMSS');
      ...
      case 5001
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            evt.class = 'CTD OFFSET PRESSURE OF THE PREVIOUS CYCLE';
            evt.label = 'Time';
            [dataEpoch, curByte] = get_bytes_pfv2(dataIn, curByte, 4);
            dataJuld = epoch2000_2_julian(dataEpoch);
            evt.valueStr = julian_2_gregorian_dec_argo(dataJuld);
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'CTD OFFSET PRESSURE OF THE PREVIOUS CYCLE: Time';
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'CTD OFFSET PRESSURE OF THE PREVIOUS CYCLE: SBE41 offset pressure (dbar)';
            dataStruct.techId = 500101;
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 5);
            dataStruct.value = num2str(data);
            tabTech = [tabTech dataStruct];
         end
      ...
      case 5002
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            evt = get_pfv2_tech_event_data_init_struct(500201);
            evt.class = 'CTD OFFSET PRESSURE OF THE PREVIOUS CYCLE';
            evt.label = 'RBRargo3 offset pressure (dbar)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 5);
            evt.valueStr = num2str(data);
            tabEvt = [tabEvt evt];
         else
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'CTD OFFSET PRESSURE OF THE PREVIOUS CYCLE: RBRargo3 offset pressure (dbar)';
            dataStruct.techId = 500201;
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 5);
            dataStruct.value = num2str(data);
            tabTech = [tabTech dataStruct];
         end
      ...
      case 5003
         if (~isempty(g_decArgo_outputCsvFileId))
            evt = get_pfv2_tech_event_data_init_struct(500300);
            ...
            evt = get_pfv2_tech_event_data_init_struct(500301);
            evt.class = 'CTD OFFSET PRESSURE';
            evt.label = 'SBE41 offset pressure (dbar)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 5);
            evt.valueStr = num2str(data);
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'CTD OFFSET PRESSURE: Time';
            dataStruct.techId = 500300;
            [dataEpoch, curByte] = get_bytes_pfv2(dataIn, curByte, 4);
            dataJuld = epoch2000_2_julian(dataEpoch);
            dataStruct.julD = dataJuld;
            dataStruct.value = datestr(dataJuld + g_decArgo_janFirst1950InMatlab, 'yyyymmddHHMMSS');
      ...
      case 5004
         if (~isempty(g_decArgo_outputCsvFileId))
            evt = get_pfv2_tech_event_data_init_struct(500400);
            evt.class = 'CTD OFFSET PRESSURE';
            evt.label = 'Time';
            [dataEpoch, curByte] = get_bytes_pfv2(dataIn, curByte, 4);
            dataJuld = epoch2000_2_julian(dataEpoch);
            ...
            evt = get_pfv2_tech_event_data_init_struct(500401);
            evt.class = 'CTD OFFSET PRESSURE';
            evt.label = 'RBRargo3 offset pressure (dbar)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 5);
            evt.valueStr = num2str(data);
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'CTD OFFSET PRESSURE: RBRargo3 offset pressure (dbar)';
            dataStruct.techId = 500401;
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 5);
            dataStruct.value = num2str(data);
            tabTech = [tabTech dataStruct];
         end
      ...
      case 6113
         if (~isempty(g_decArgo_outputCsvFileId))
            evt = get_pfv2_tech_event_data_init_struct(611300);
            evt.class = 'GNSS FIX SELF TEST';
            evt.label = 'Result flag (0: error, 1: ok, 3: not tested)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 1);
            evt.valueStr = num2str(data);
            tabEvt = [tabEvt evt];
         else
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'GNSS FIX SELF TEST: Result flag (0: error, 1: ok, 3: not tested)';
            dataStruct.techId = 611300;
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 1);
            if (data ~= 3)
               data = (data == 0);
            end
      ...
      case 6114
         if (~isempty(g_decArgo_outputCsvFileId))
            evt = get_pfv2_tech_event_data_init_struct(611400);
            evt.class = 'SBE41 OFFSET PRESSURE SELF TEST';
            evt.label = 'Result flag (0: error, 1: ok, 3: not tested)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 1);
            evt.valueStr = num2str(data);
            tabEvt = [tabEvt evt];

            evt = get_pfv2_tech_event_data_init_struct(611401);
            evt.class = 'SBE41 OFFSET PRESSURE SELF TEST';
            evt.label = 'Offset pressure (dbar)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 5);
            evt.valueStr = sprintf('%.1f', data);
            tabEvt = [tabEvt evt];
         else
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'SBE41 OFFSET PRESSURE SELF TEST: Result flag (0: error, 1: ok, 3: not tested)';
            dataStruct.techId = 611400;
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 1);
            if (data ~= 3)
               data = (data == 0);
            end
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'SBE41 OFFSET PRESSURE SELF TEST: Offset pressure (dbar)';
            dataStruct.techId = 611401;
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 5);
            dataStruct.value = sprintf('%.1f', data);
            tabTech = [tabTech dataStruct];
         end
      ...
      case 6115
         if (~isempty(g_decArgo_outputCsvFileId))
            evt = get_pfv2_tech_event_data_init_struct(611500);
            evt.class = 'RBR OFFSET PRESSURE SELF TEST';
            evt.label = 'Result flag (0: error, 1: ok, 3: not tested)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 1);
            evt.valueStr = num2str(data);
            tabEvt = [tabEvt evt];

            evt = get_pfv2_tech_event_data_init_struct(611501);
            evt.class = 'RBR OFFSET PRESSURE SELF TEST';
            evt.label = 'Offset pressure (dbar)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 5);
            evt.valueStr = sprintf('%.1f', data);
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'RBR OFFSET PRESSURE SELF TEST: Result flag (0: error, 1: ok, 3: not tested)';
            dataStruct.techId = 611500;
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 1);
            if (data ~= 3)
               data = (data == 0);
            end
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'RBR OFFSET PRESSURE SELF TEST: Offset pressure (dbar)';
            dataStruct.techId = 611501;
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 5);
            dataStruct.value = sprintf('%.1f', data);
            tabTech = [tabTech dataStruct];
         end
      ...
      case 6116
         if (~isempty(g_decArgo_outputCsvFileId))
            evt = get_pfv2_tech_event_data_init_struct(611600);
            evt.class = 'SBE61 SERIAL NUMBER';
            evt.label = 'Result flag (0: error, 1: ok)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 1);
            evt.valueStr = num2str(data);
            tabEvt = [tabEvt evt];

            evt = get_pfv2_tech_event_data_init_struct(611601);
            evt.class = 'SBE61 SERIAL NUMBER';
            evt.label = 'Serial number';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(data);
            tabEvt = [tabEvt evt];
            if (isempty(g_decArgo_metaFromTech.techId) || ~any(g_decArgo_metaFromTech.techId == 611601))
               g_decArgo_metaFromTech.techId = [g_decArgo_metaFromTech.techId 611601];
               g_decArgo_metaFromTech.value = [g_decArgo_metaFromTech.value {evt.valueStr}];
            end
         else
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNum, cycleNum);
            dataStruct.label = 'SBE61 SERIAL NUMBER: Result flag (0: ok, 1: error)';
            dataStruct.techId = 611600;
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 1);
            dataStruct.value = num2str(data == 0);
            tabTech = [tabTech dataStruct];
      ...
      case 9000
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            dataJuld = epoch2000_2_julian(dataEpoch);
            evt.valueStr = julian_2_gregorian_dec_argo(dataJuld);
            tabEvtTmp = [tabEvtTmp evt];

            evt = get_pfv2_tech_event_data_init_struct(900001);
            evt.class = 'PROFILE SAMPLES PROCESSED';
            evt.label = 'Mission number';
            [missionNumber, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(missionNumber);
            tabEvtTmp = [tabEvtTmp evt];
            if (missionNumber ~= missionNum)
               fprintf('WARNING: Float #%d: Delayed TECH event #9000 (mission number %d while %d is expected) in TECH file: %s\n', ...
                  g_decArgo_floatNum, ...
                  missionNumber, missionNum, a_fileName);
            end

            evt = get_pfv2_tech_event_data_init_struct(900002);
            evt.class = 'PROFILE SAMPLES PROCESSED';
            evt.label = 'Cycle number';
            [cycleNumber, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(cycleNumber);
            tabEvtTmp = [tabEvtTmp evt];
            if (cycleNumber ~= cycleNum)
               fprintf('WARNING: Float #%d: Delayed TECH event #9000 (cycle number %d while %d is expected) in TECH file: %s\n', ...
                  g_decArgo_floatNum, ...
                  cycleNumber, cycleNum, a_fileName);
            end
            ...
            evt = get_pfv2_tech_event_data_init_struct(900013);
            evt.class = 'PROFILE SAMPLES PROCESSED';
            ...
            evt = get_pfv2_tech_event_data_init_struct(900014);
            evt.class = 'PROFILE SAMPLES PROCESSED';
            ...
            if ((missionNumber ~= missionNum) || (cycleNumber ~= cycleNum))
               tabEvtBis = [tabEvtBis; [missionNumber cycleNumber {tabEvtTmp}]];
            else
               tabEvt = [tabEvt tabEvtTmp];
            end
         else
            [~, curByte] = get_bytes_pfv2(dataIn, curByte, 4);
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNumber, cycleNumber);
            ...
            tabTechTmp = [tabTechTmp dataStruct];
            ...
            if ((missionNumber ~= missionNum) || (cycleNumber ~= cycleNum))
               tabTechBis = [tabTechBis; [missionNumber cycleNumber {tabTechTmp}]];
            else
               tabTech = [tabTech tabTechTmp];
            end
         end
      ...
      case 9001
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            tabEvtTmp = [tabEvtTmp evt];
            if (missionNumber ~= missionNum)
               fprintf('WARNING: Float #%d: Delayed TECH event #9001 (mission number %d while %d is expected) in TECH file: %s\n', ...
                  g_decArgo_floatNum, ...
                  missionNumber, missionNum, a_fileName);
            end
            ...
            tabEvtTmp = [tabEvtTmp evt];
            if (cycleNumber ~= cycleNum)
               fprintf('WARNING: Float #%d: Delayed TECH event #9001 (cycle number %d while %d is expected) in TECH file: %s\n', ...
                  g_decArgo_floatNum, ...
                  cycleNumber, cycleNum, a_fileName);
            end
            ...
            evt = get_pfv2_tech_event_data_init_struct(900104);
            evt.class = 'DRIFT SAMPLES PROCESSED';
            evt.label = 'Sensor number (1:SBE41, 2:AANDERAA4330, 3:RBR)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 1);
            evt.valueStr = num2str(data);
            ...
            evt = get_pfv2_tech_event_data_init_struct(900105);
            evt.class = 'DRIFT SAMPLES PROCESSED';
      ...
      case 9002
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            evt = get_pfv2_tech_event_data_init_struct(900200);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Time';
            [dataEpoch, curByte] = get_bytes_pfv2(dataIn, curByte, 4);
            dataJuld = epoch2000_2_julian(dataEpoch);
            evt.valueStr = julian_2_gregorian_dec_argo(dataJuld);
            ...
            evt = get_pfv2_tech_event_data_init_struct(900201);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Mission number';
            [missionNumber, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(missionNumber);
            tabEvtTmp = [tabEvtTmp evt];
            if (missionNumber ~= missionNum)
               fprintf('WARNING: Float #%d: Delayed TECH event #9002 (mission number %d while %d is expected) in TECH file: %s\n', ...
                  g_decArgo_floatNum, ...
                  missionNumber, missionNum, a_fileName);
            end

            evt = get_pfv2_tech_event_data_init_struct(900202);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Cycle number';
            [cycleNumber, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(cycleNumber);
            tabEvtTmp = [tabEvtTmp evt];
            if (cycleNumber ~= cycleNum)
               fprintf('WARNING: Float #%d: Delayed TECH event #9002 (cycle number %d while %d is expected) in TECH file: %s\n', ...
                  g_decArgo_floatNum, ...
                  cycleNumber, cycleNum, a_fileName);
            end

            evt = get_pfv2_tech_event_data_init_struct(900203);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Phase number (0:descent to park, 2:descent to prof, 4:ascent)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 1);
            evt.valueStr = num2str(data);
            ...
            evt = get_pfv2_tech_event_data_init_struct(900204);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Sensor number (1:SBE41, 2:AANDERAA4330, 3:RBR)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 1);
            evt.valueStr = num2str(data);
            ...
            evt = get_pfv2_tech_event_data_init_struct(900205);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone1: number of raw data';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(data);
            tabEvtTmp = [tabEvtTmp evt];

            evt = get_pfv2_tech_event_data_init_struct(900206);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone1: number of decimated data';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(data);
            tabEvtTmp = [tabEvtTmp evt];

            evt = get_pfv2_tech_event_data_init_struct(900207);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone1: number of processed data';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(data);
            tabEvtTmp = [tabEvtTmp evt];

            evt = get_pfv2_tech_event_data_init_struct(900208);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone1: treatment type (0x00:raw, 0x80:raw decimated, 0x81:average decimated)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 1);
            evt.valueStr = ['0x' dec2hex(data)];
            ...
            evt = get_pfv2_tech_event_data_init_struct(900209);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone2: number of raw data';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(data);
            tabEvtTmp = [tabEvtTmp evt];

            evt = get_pfv2_tech_event_data_init_struct(900210);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone2: number of decimated data';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(data);
            tabEvtTmp = [tabEvtTmp evt];

            evt = get_pfv2_tech_event_data_init_struct(900211);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone2: number of processed data';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(data);
            tabEvtTmp = [tabEvtTmp evt];

            evt = get_pfv2_tech_event_data_init_struct(900212);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone2: treatment type (0x00:raw, 0x80:raw decimated, 0x81:average decimated)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 1);
            evt.valueStr = ['0x' dec2hex(data)];
            ...
            evt = get_pfv2_tech_event_data_init_struct(900213);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone3: number of raw data';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(data);
            tabEvtTmp = [tabEvtTmp evt];

            evt = get_pfv2_tech_event_data_init_struct(900214);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone3: number of decimated data';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(data);
            tabEvtTmp = [tabEvtTmp evt];

            evt = get_pfv2_tech_event_data_init_struct(900215);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone3: number of processed data';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(data);
            tabEvtTmp = [tabEvtTmp evt];

            evt = get_pfv2_tech_event_data_init_struct(900216);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone3: treatment type (0x00:raw, 0x80:raw decimated, 0x81:average decimated)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 1);
            evt.valueStr = ['0x' dec2hex(data)];
            ...
            evt = get_pfv2_tech_event_data_init_struct(900217);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone4: number of raw data';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(data);
            tabEvtTmp = [tabEvtTmp evt];

            evt = get_pfv2_tech_event_data_init_struct(900218);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone4: number of decimated data';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(data);
            tabEvtTmp = [tabEvtTmp evt];

            evt = get_pfv2_tech_event_data_init_struct(900219);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone4: number of processed data';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(data);
            tabEvtTmp = [tabEvtTmp evt];

            evt = get_pfv2_tech_event_data_init_struct(900220);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone4: treatment type (0x00:raw, 0x80:raw decimated, 0x81:average decimated)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 1);
            evt.valueStr = ['0x' dec2hex(data)];
            ...
            evt = get_pfv2_tech_event_data_init_struct(900221);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone5: number of raw data';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(data);
            tabEvtTmp = [tabEvtTmp evt];

            evt = get_pfv2_tech_event_data_init_struct(900222);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone5: number of decimated data';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(data);
            tabEvtTmp = [tabEvtTmp evt];

            evt = get_pfv2_tech_event_data_init_struct(900223);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone5: number of processed data';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            evt.valueStr = num2str(data);
            tabEvtTmp = [tabEvtTmp evt];

            evt = get_pfv2_tech_event_data_init_struct(900224);
            evt.class = 'PROFILE DECIMATED RAW DATA PROCESSED';
            evt.label = 'Zone5: treatment type (0x00:raw, 0x80:raw decimated, 0x81:average decimated)';
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 1);
            evt.valueStr = ['0x' dec2hex(data)];
            ...
            switch (phaseNum)
               case {0} % descent to park
                  phaseName = 'descent to park';
                  techId = 900225:900227;
               case {2} % descent to prof
                  phaseName = 'descent to prof';
                  techId = 900228:900230;
               case {4} % ascent
                  phaseName = 'ascent';
                  techId = 900231:900233;
            end
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNumber, cycleNumber);
            dataStruct.label = sprintf('PROFILE DECIMATED RAW DATA PROCESSED: Zone1: number of raw data for sensor ''%s'' during %s', shortSensorName, phaseName);
            dataStruct.techId = techId(1);
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            dataStruct.value = num2str(data);
            dataStruct.sensorNum = sensorNum;
            dataStruct.shortSensorName = shortSensorName;
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNumber, cycleNumber);
            dataStruct.label = sprintf('PROFILE DECIMATED RAW DATA PROCESSED: Zone1: number of decimated data for sensor ''%s'' during %s', shortSensorName, phaseName);
            dataStruct.techId = techId(2);
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            dataStruct.value = num2str(data);
            dataStruct.sensorNum = sensorNum;
            dataStruct.shortSensorName = shortSensorName;
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNumber, cycleNumber);
            dataStruct.label = sprintf('PROFILE DECIMATED RAW DATA PROCESSED: Zone1: number of processed data for sensor ''%s'' during %s', shortSensorName, phaseName);
            dataStruct.techId = techId(3);
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            dataStruct.value = num2str(data);
            dataStruct.sensorNum = sensorNum;
            dataStruct.shortSensorName = shortSensorName;
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNumber, cycleNumber);
            dataStruct.label = sprintf('PROFILE DECIMATED RAW DATA PROCESSED: Zone2: number of raw data for sensor ''%s'' during %s', shortSensorName, phaseName);
            dataStruct.techId = techId(1);
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            dataStruct.value = num2str(data);
            dataStruct.sensorNum = sensorNum;
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNumber, cycleNumber);
            dataStruct.label = sprintf('PROFILE DECIMATED RAW DATA PROCESSED: Zone2: number of decimated data for sensor ''%s'' during %s', shortSensorName, phaseName);
            dataStruct.techId = techId(2);
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            dataStruct.value = num2str(data);
            dataStruct.sensorNum = sensorNum;
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNumber, cycleNumber);
            dataStruct.label = sprintf('PROFILE DECIMATED RAW DATA PROCESSED: Zone2: number of processed data for sensor ''%s'' during %s', shortSensorName, phaseName);
            dataStruct.techId = techId(3);
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            dataStruct.value = num2str(data);
            dataStruct.sensorNum = sensorNum;
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNumber, cycleNumber);
            dataStruct.label = sprintf('PROFILE DECIMATED RAW DATA PROCESSED: Zone3: number of raw data for sensor ''%s'' during %s', shortSensorName, phaseName);
            dataStruct.techId = techId(1);
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            dataStruct.value = num2str(data);
            dataStruct.sensorNum = sensorNum;
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNumber, cycleNumber);
            dataStruct.label = sprintf('PROFILE DECIMATED RAW DATA PROCESSED: Zone3: number of decimated data for sensor ''%s'' during %s', shortSensorName, phaseName);
            dataStruct.techId = techId(2);
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            dataStruct.value = num2str(data);
            dataStruct.sensorNum = sensorNum;
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNumber, cycleNumber);
            dataStruct.label = sprintf('PROFILE DECIMATED RAW DATA PROCESSED: Zone3: number of processed data for sensor ''%s'' during %s', shortSensorName, phaseName);
            dataStruct.techId = techId(3);
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            dataStruct.value = num2str(data);
            dataStruct.sensorNum = sensorNum;
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNumber, cycleNumber);
            dataStruct.label = sprintf('PROFILE DECIMATED RAW DATA PROCESSED: Zone4: number of raw data for sensor ''%s'' during %s', shortSensorName, phaseName);
            dataStruct.techId = techId(1);
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            dataStruct.value = num2str(data);
            dataStruct.sensorNum = sensorNum;
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNumber, cycleNumber);
            dataStruct.label = sprintf('PROFILE DECIMATED RAW DATA PROCESSED: Zone4: number of decimated data for sensor ''%s'' during %s', shortSensorName, phaseName);
            dataStruct.techId = techId(2);
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            dataStruct.value = num2str(data);
            dataStruct.sensorNum = sensorNum;
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNumber, cycleNumber);
            dataStruct.label = sprintf('PROFILE DECIMATED RAW DATA PROCESSED: Zone4: number of processed data for sensor ''%s'' during %s', shortSensorName, phaseName);
            dataStruct.techId = techId(3);
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            dataStruct.value = num2str(data);
            dataStruct.sensorNum = sensorNum;
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNumber, cycleNumber);
            dataStruct.label = sprintf('PROFILE DECIMATED RAW DATA PROCESSED: Zone5: number of raw data for sensor ''%s'' during %s', shortSensorName, phaseName);
            dataStruct.techId = techId(1);
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            dataStruct.value = num2str(data);
            dataStruct.sensorNum = sensorNum;
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNumber, cycleNumber);
            dataStruct.label = sprintf('PROFILE DECIMATED RAW DATA PROCESSED: Zone5: number of decimated data for sensor ''%s'' during %s', shortSensorName, phaseName);
            dataStruct.techId = techId(2);
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            dataStruct.value = num2str(data);
            dataStruct.sensorNum = sensorNum;
            ...
            dataStruct = get_pfv2_tech_data_init_struct(fileType, missionNumber, cycleNumber);
            dataStruct.label = sprintf('PROFILE DECIMATED RAW DATA PROCESSED: Zone5: number of processed data for sensor ''%s'' during %s', shortSensorName, phaseName);
            dataStruct.techId = techId(3);
            [data, curByte] = get_bytes_pfv2(dataIn, curByte, 2);
            dataStruct.value = num2str(data);
            dataStruct.sensorNum = sensorNum;
...
switch (a_alarmType)
   ...
   case 17
      o_alarmExplanation = 'Treatment timeout';
   case 18
   ...
   case 30
      o_alarmExplanation = 'ISA dectection';
   case 31
      o_alarmExplanation = 'Satellite mask';
   case 32
   ...
   case 35
      o_alarmExplanation = 'Failure on initialization of CTD RBR reset offset';
   case 36
      o_alarmExplanation = 'Aborted profile';
   case 37
      o_alarmExplanation = 'Ice - Aborted profile';
   case 38
      o_alarmExplanation = 'Ice - Breakup period';
   case 39
      o_alarmExplanation = 'Ice - Forced ascent';
   otherwise
