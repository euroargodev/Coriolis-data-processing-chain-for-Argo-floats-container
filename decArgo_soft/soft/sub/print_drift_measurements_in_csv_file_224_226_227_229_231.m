...
% ------------------------------------------------------------------------------
function print_drift_measurements_in_csv_file_224_226_227_229_231( ...
   a_parkDate, a_parkDateAdj, a_parkTransDate, ...
   a_parkPres, a_parkTemp, a_parkSal, a_parkSalCor, a_parkTempCndc, ...
   a_parkC1PhaseDoxy, a_parkC2PhaseDoxy, a_parkTempDoxy, a_parkDoxy)
...
if (~isempty(a_parkPres))
   ...
   if (isempty(a_parkC1PhaseDoxy))
      if (~isempty(a_parkSal) && isempty(a_parkSalCor) && isempty(a_parkTempCndc))
      ...
      elseif (~isempty(a_parkSal) && isempty(a_parkSalCor) && ~isempty(a_parkTempCndc))
      ...
      elseif (~isempty(a_parkSal) && ~isempty(a_parkSalCor) && isempty(a_parkTempCndc))
         ...
         fprintf(g_decArgo_outputCsvFileId, '%d; %d; Drift; Description; Float time; UTC time; PRES (dbar); TEMP (degC); PSAL (PSU); PSAL_DynamicCorrection (PSU)\n', ...
            g_decArgo_floatNum, g_decArgo_cycleNum);
         ...
         for idMes = 1:length(a_parkDate)
            ...
            fprintf(g_decArgo_outputCsvFileId, '%d; %d; Drift; Drift meas. #%d; %s; %s (%c); %.1f; %.3f; %.3f; %.3f\n', ...
               g_decArgo_floatNum, g_decArgo_cycleNum, ...
               idMes, mesDateStr, mesDateAdjStr, trans, ...
               a_parkPres(idMes), a_parkTemp(idMes), a_parkSal(idMes), a_parkSalCor(idMes));
         end
         
      elseif (isempty(a_parkSal) && ~isempty(a_parkSalCor) && isempty(a_parkTempCndc))

         fprintf(g_decArgo_outputCsvFileId, '%d; %d; Drift; Description; Float time; UTC time; PRES (dbar); TEMP (degC); PSAL_DynamicCorrection (PSU)\n', ...
            g_decArgo_floatNum, g_decArgo_cycleNum);
         ...
         for idMes = 1:length(a_parkDate)
            ...
            fprintf(g_decArgo_outputCsvFileId, '%d; %d; Drift; Drift meas. #%d; %s; %s (%c); %.1f; %.3f; %.3f\n', ...
               g_decArgo_floatNum, g_decArgo_cycleNum, ...
               idMes, mesDateStr, mesDateAdjStr, trans, ...
               a_parkPres(idMes), a_parkTemp(idMes), a_parkSalCor(idMes));
         end
