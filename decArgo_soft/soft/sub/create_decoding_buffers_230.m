...
% ------------------------------------------------------------------------------
function [o_decodedData] = create_decoding_buffers_230(a_decodedData, a_decoderId)
...
for sesNum = sessionList
   ...
   if (~any(ismember(tabPackType(idForSession), [30:33]))) % transmission after a deep cycle
...
idPackDesc = find(a_tabPackType(a_idForCheck) == 30);
idPackDrift = find(a_tabPackType(a_idForCheck) == 31);
idPackAsc = find(a_tabPackType(a_idForCheck) == 32);
idPackNearSurface = find(a_tabPackType(a_idForCheck) == 33);
idPackInAir = find(a_tabPackType(a_idForCheck) == 34);
