...
% ------------------------------------------------------------------------------
function [o_decodedData] = create_decoding_buffers_224_226_227_231(a_decodedData, a_decoderId)
...
if (ismember(g_decArgo_floatNum, [ ...
      4903632, 7902238, 3902607]))
   switch g_decArgo_floatNum
      case 4903632
         % cycle #155 data are separated
         id1 = find(tabCyNum == 155);
...
% check buffer completion
packType = a_tabPackType(a_idForCheck);
idPackTech1 = find(packType == 0);
idPackTech2 = find(packType == 4);
idPackProg = find(packType == 5);
% for 5.49 floats, float parameter message is transmitted only when a parameter
% has been modified
if (ismember(a_decoderId, [224, 226, 227, 231]))
   idPackProg = -1;
end
idPackDesc = find((packType == 1) | (packType == 8) | (packType == 15) | (packType == 20) | ...
   (packType == 25) | (packType == 31) | (packType == 36) | (packType == 41));
idPackDrift = find((packType == 2) | (packType == 9) | (packType == 16) | (packType == 21) | ...
   (packType == 27) | (packType == 32) | (packType == 37) | (packType == 42));
idPackAsc = find((packType == 3) | (packType == 10) | (packType == 17) | (packType == 22) | ...
   (packType == 28) | (packType == 33) | (packType == 38) | (packType == 43));
...
switch (a_decoderId)
   case {224, 226, 227, 231}
      switch (a_packType)
         ...
         case {1, 8, 15, 20, 25, 31, 36, 41}
            o_packTypeDesc = 'Desc meas';
         case {2, 9, 16, 21, 27, 32, 37, 42}
            o_packTypeDesc = 'Drift meas';
         case {3, 10, 17, 22, 28, 33, 38, 43}
         ...
         case {11, 13, 18, 23, 29, 34, 39, 44}
            o_packTypeDesc = 'NS meas';
         case {12, 14, 19, 24, 30, 35, 40, 45}
