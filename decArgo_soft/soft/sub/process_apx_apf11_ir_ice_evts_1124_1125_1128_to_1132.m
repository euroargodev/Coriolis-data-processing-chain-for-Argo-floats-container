...
%
% SYNTAX :
%  [o_iceDetection] = process_apx_apf11_ir_ice_evts_1124_1125_1128_to_1132(a_events)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function [o_iceDetection] = process_apx_apf11_ir_ice_evts_1124_1125_1128_to_1132(a_events)
...
PATTERN_MISSION_ICE_DESCENT = 'starting ice descent cycle';
PATTERN_COMMS_FOUND_SKY = 'found sky';
PATTERN_COMMS_NO_SKY = 'no sky';
...
for idEv = 1:length(events)
   ...
   if (strcmp(evt.functionName, 'ICE'))
      if (any(strfind(dataStr, PATTERN_THERMAL_DETECT_SAMPLE)))
         ...
         sample = sample{:};
         if (length(sample) == 3) % T is missing in 3902686 #52
            sampleNum = sample{1};
      ...
      elseif (any(strfind(dataStr, PATTERN_EVALUATE_MEDIAN_TEMPERATURE_FALSE)))
         ...
         if (any(strfind(median{3}, PATTERN_THERMAL_DETECT_MEDIAN_TEMP_VALUE)))
            ...
            thermalDetect.medianTemp = medianTempValue;
         end
   ...
   elseif (strcmp(evt.functionName, 'mission_state'))
      ...
      elseif (any(strfind(dataStr, PATTERN_MISSION_ICE_DESCENT)))

         idF = strfind(dataStr, PATTERN_MISSION_ICE_DESCENT);
         iceCycleNum = str2double(dataStr(idF+length(PATTERN_MISSION_ICE_DESCENT)+1:end));
         iceDetection.iceCycleNumbers = [iceDetection.iceCycleNumbers iceCycleNum];
      end
   elseif (strcmp(evt.functionName, 'COMMS'))
      if (any(strfind(dataStr, PATTERN_COMMS_FOUND_SKY)))

         if (~isempty(iceDetection))
            iceDetection.foundSkyFlag = 1;
         end

      elseif (any(strfind(dataStr, PATTERN_COMMS_NO_SKY)))
