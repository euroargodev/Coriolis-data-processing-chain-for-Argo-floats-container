...
%    create_config_param_names_ir_rudics_cts5_139_140
%
% INPUT PARAMETERS :
%
% OUTPUT PARAMETERS :
...
configInfoList = [ ...
   ...
   {'TECHNICAL'} {[0:25 28 29 30]} {[112:134 197 198 286 289 290 291]}; ...
   ...
   {'ISA'} {1:4} {266:269}; ...
   {'EVENT_DRIVEN'} {1} {294}; ...
   {'EVENT_RAIN'} {1:12} {[295:299 500:506]}; ...
   {'EVENT_WIND'} {1:12} {507:518}; ...
   {'IRIDIUM_RUDICS'} {[0:1 3:9]} {[301:303 166:169 277 285]}; ...
   {'MOTOR'} {0:3} {[304 170 292 293]}; ...
...
for idConfig = 1:length(configInfoList)
   ...
   elseif (strcmp(section, 'SENSOR_'))
      for idS = 1:length(apmtSensorList)
         ...
         paramIdList = 175:183;
         sensorNum = idS;
         if (ismember(idS, nonArgoSensorIdList)) % <short_sensor_name> = non Argo sensor name
         ...
         elseif (strcmp(apmtSensorList{idS}, 'Pal')) % <short_sensor_name> = 'Pal'
            paramNumList = [54:58 61:68];
            paramIdList = [519:531];
            for idP = 1:length(paramNumList)
               decConfNames{end+1} = sprintf('CONFIG_APMT_%s%02d_P%02d', section, sensorNum, paramNumList(idP));
               idParamName = find(g_decArgo_outputNcConfParamId == paramIdList(idP));
         ...
         elseif (strcmp(apmtSensorList{idS}, 'Tridente')) % <short_sensor_name> = 'Tridente'
            paramNumList = 54;
            paramIdList = 532;
            for idP = 1:length(paramNumList)
               decConfNames{end+1} = sprintf('CONFIG_APMT_%s%02d_P%02d', section, sensorNum, paramNumList(idP));
