   ...
   if ((phaseNum == g_decArgo_phaseDsc2Prk) || ...
      ...
      if (phaseNum == g_decArgo_phaseAscProf)
         ...
         if (isempty(subSurfacePres))
            ...
            if (ismember(a_decoderId, [302, 303]))
               ...
               [presCutOffProfCfg] = config_get_value_ir_rudics_sbd2(cycleNum, profNum, 'CONFIG_PC_0_1_3');
               if (~isempty(presCutOffProfCfg) && ~isnan(presCutOffProfCfg))
                  % CONFIG_PC_0_1_3 is CTD pump cut-off pressure we should add
                  % Poverlap = 0.5 dbar because the treatment cannot be 'raw'
                  % for this float type
