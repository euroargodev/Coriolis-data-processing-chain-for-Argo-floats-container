% ------------------------------------------------------------------------------
% Decode PAL (high dynamic range) data transmitted by a CTS5-USEA float.
%
% SYNTAX :
%  [o_palData] = decode_apmt_pal_hdr(a_data, a_lastByteNum, a_inputFilePathName)
%
% INPUT PARAMETERS :
...
%
% OUTPUT PARAMETERS :
%   o_palData : PAL (high dynamic range) decoded data
%
% EXAMPLES :
%
% SEE ALSO :
...
% ------------------------------------------------------------------------------
function [o_palData] = decode_apmt_pal_hdr(a_data, a_lastByteNum, a_inputFilePathName)
...
while (currentByte <= lastByteNum)
   ...
   if ((currentPhaseNum ~= -1) && (currentTreatNum ~= -1))
      ...
      data(3) = rawData(3);
      data(4) = rawData(4)/5;
      data(5:end) = (rawData(5:end) + data(4))/2;
