...
%    decode_system_log_apx_apf11_ir_1128_to_1132(a_systemLogFileList, a_cycleTimeData, a_presOffsetData, a_techData, a_decoderId)
%
% INPUT PARAMETERS :
...
for idFile = 1:length(a_systemLogFileList)
   ...
   if (~isempty(g_decArgo_outputCsvFileId) && (g_decArgo_cycleNum == 0))
      ...
      if (~isempty(idEvts))
         metaData = process_apx_apf11_ir_meta_data_evts_1128_to_1132(events(idEvts));
   ...
   if (~isempty(idEvts))
      [o_cycleTimeData] = process_apx_apf11_ir_time_evts_1128_to_1132(events(idEvts), o_cycleTimeData);
