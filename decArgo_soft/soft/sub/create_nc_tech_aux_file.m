...
% generate NetCDF-4 flag for TECH_AUX file
...
for idNM = 1:length(a_tabTechNMeas)
   ...
   if (~isempty(nMeas.tabMeas))
      ...
      if (~isempty(measParamList))
         ...
         measParamNameList = regexprep(measParamNameList, 'TECH_AUX_', 'TECH_');
...
if (g_decArgo_netCDF4FlagForTechAux)
...
try
   ...
   if (nbMeasParam > 0)
      technicalMeasParametersVarId = netcdf.defVar(fCdf, 'TECHNICAL_MEASUREMENT_PARAMETERS', 'NC_CHAR', fliplr([nTechMeasParamDimId string256DimId]));
      if (g_decArgo_netCDF4FlagForTechAux)
         netcdf.defVarDeflate(fCdf, technicalMeasParametersVarId, SHUFFLE_FLAG, true, g_decArgo_netCDFDeflateLevel);
      end
   ...
   if (g_decArgo_netCDF4FlagForTechAux)
      netcdf.defVarDeflate(fCdf, technicalParameterNameVarId, SHUFFLE_FLAG, true, g_decArgo_netCDFDeflateLevel);
   end
   ...
   if (g_decArgo_netCDF4FlagForTechAux)
      netcdf.defVarDeflate(fCdf, technicalParameterValueVarId, SHUFFLE_FLAG, true, g_decArgo_netCDFDeflateLevel);
   end
   ...
   if (~isempty(techAuxParamLabelList))
      ...
      if (g_decArgo_netCDF4FlagForTechAux)
         netcdf.defVarDeflate(fCdf, refTechAuxParamLabelVarId, SHUFFLE_FLAG, true, g_decArgo_netCDFDeflateLevel);
      end
      ...
      if (g_decArgo_netCDF4FlagForTechAux)
         netcdf.defVarDeflate(fCdf, refTechAuxParamDescriptionVarId, SHUFFLE_FLAG, true, g_decArgo_netCDFDeflateLevel);
      end
   ...
   if (~isempty(a_tabTechNMeas))
      ...
      if (g_decArgo_netCDF4FlagForTechAux)
         netcdf.defVarDeflate(fCdf, cycleNumberMeasVarId, SHUFFLE_FLAG, true, g_decArgo_netCDFDeflateLevel);
      end
      ...
      for idNM = 1:length(a_tabTechNMeas)
         ...
         for idM = 1:length(nMeas.tabMeas)
            ...
            for idParam = 1:length(measParamList)
               ...
               measParamName = regexprep(measParamName, 'TECH_AUX_', 'TECH_');
