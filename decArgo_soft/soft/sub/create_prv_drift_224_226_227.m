...
%
% SYNTAX :
% [o_parkDate, o_parkTransDate, o_parkPres, o_parkTemp, o_parkSal, ...
%   o_parkSalCor, o_parkTempCndc] = create_prv_drift_224_226_227(a_dataCTDRbr, a_refDay)
%
% INPUT PARAMETERS :
...
%   o_parkSalCor    : drift meas PSAL_DynamicCorrection
...
% ------------------------------------------------------------------------------
function [o_parkDate, o_parkTransDate, o_parkPres, o_parkTemp, o_parkSal, ...
   o_parkSalCor, o_parkTempCndc] = create_prv_drift_224_226_227(a_dataCTDRbr, a_refDay)
...
idDrift = find(ismember(a_dataCTDRbr(:, 1), [2, 16, 27, 37]));
typeList = unique(a_dataCTDRbr(idDrift, 1));
      ...
      switch (type)
         ...
         case 27
            ...
            for idMeas = 1:11
               ...
               o_parkTemp = [o_parkTemp; data(idMeas+11*3)];
               o_parkSal = [o_parkSal; data(idMeas+11*4)];
               o_parkSalCor = [o_parkSalCor; data(idMeas+11*5)];
            end
         case 37
            ...
            for idMeas = 1:11
               ...
               o_parkPres = [o_parkPres; data(idMeas+11*2)];
               o_parkTemp = [o_parkTemp; data(idMeas+11*3)];
               o_parkSalCor = [o_parkSalCor; data(idMeas+11*4)];
            end
      end
   end
end
...
if (~isempty(o_parkSalCor))
   o_parkSalCor = o_parkSalCor(idSorted);
end
if (~isempty(o_parkTempCndc))
