...
%
% SYNTAX :
%  [o_tabTrajNMeas, o_tabTrajNCycle, o_tabTechNMeas] = process_trajectory_data_224_226_227( ...
...
%   a_parkSalCor          : drift meas PSAL_DynamicCorrection
...
%   a_nearSurfSalCor      : "near surface" profile PSAL_DynamicCorrection
...
%   a_inAirSalCor         : "in air" profile PSAL_DynamicCorrection
...
% ------------------------------------------------------------------------------
function [o_tabTrajNMeas, o_tabTrajNCycle, o_tabTechNMeas] = process_trajectory_data_224_226_227( ...
   a_cycleNum, a_deepCycle, ...
   a_gpsData, a_iridiumMailData, ...
   a_cycleTimeData, ...
   a_tabTech1, a_tabTech2, ...
...
if (~isempty(a_inAirDate))
   ...
   a_inAirPres(a_inAirPres == g_decArgo_presDef) = paramPres.fillValue;
   a_inAirTemp(a_inAirTemp == g_decArgo_tempDef) = paramTemp.fillValue;
   if (~isempty(a_inAirSal))
      a_inAirSal(a_inAirSal == g_decArgo_salDef) = paramSal.fillValue;
   end
   if (~isempty(a_inAirSalCor))
      a_inAirSalCor(a_inAirSalCor == g_decArgo_salDef) = paramSalCor.fillValue;
   end
   if (~isempty(a_inAirTempCndc))
      a_inAirTempCndc(a_inAirTempCndc == g_decArgo_tempDef) = paramTempCndc.fillValue;
   end
   if (~isempty(a_inAirC1PhaseDoxy))
      a_inAirC1PhaseDoxy(a_inAirC1PhaseDoxy == g_decArgo_c1C2PhaseDoxyDef) = paramC1PhaseDoxy.fillValue;
      a_inAirC2PhaseDoxy(a_inAirC2PhaseDoxy == g_decArgo_c1C2PhaseDoxyDef) = paramC2PhaseDoxy.fillValue;
      a_inAirTempDoxy(a_inAirTempDoxy == g_decArgo_tempDoxyDef) = paramTempDoxy.fillValue;
      a_inAirPpoxDoxy(a_inAirPpoxDoxy == g_decArgo_doxyDef) = paramPpoxDoxy.fillValue;
   end
   ...
   for idMeas = 1:length(a_inAirPres)
      ...
      % add parameter variables and data to the structure
      measStruct.paramList = [paramPres paramTemp];
      measStruct.paramData = [a_inAirPres(idMeas) a_inAirTemp(idMeas)];
      if (~isempty(a_inAirSal))
         measStruct.paramList = [measStruct.paramList paramSal];
         measStruct.paramData = [measStruct.paramData a_inAirSal(idMeas)];
      end
      if (~isempty(a_inAirTempCndc))
         measStruct.paramList = [measStruct.paramList paramTempCndc];
         measStruct.paramData = [measStruct.paramData a_inAirTempCndc(idMeas)];
      end
      if (~isempty(a_inAirC1PhaseDoxy))
         measStruct.paramList = [measStruct.paramList paramC1PhaseDoxy paramC2PhaseDoxy paramTempDoxy paramPpoxDoxy];
         measStruct.paramData = [measStruct.paramData a_inAirC1PhaseDoxy(idMeas) a_inAirC2PhaseDoxy(idMeas) a_inAirTempDoxy(idMeas) a_inAirPpoxDoxy(idMeas)];
      end
      ...
      if (~isempty(a_inAirSalCor))
         ...
         measStructAux.paramList = [paramPres paramSalCor];
...
if (a_deepCycle == 1)
   ...
   for idProf = 1:length(a_tabProfiles)
      ...
      for idMeas = 1:length(profDates)
         if (profDates(idMeas) ~= dateFillValue)
            ...
            measStruct.sensorNumber = profile.sensorNumber; % for PROF or PROF_AUX files
   ...
   if (~isempty(a_parkPres))
      ...
      % convert decoder default values to netCDF fill values
      a_parkPres(a_parkPres == g_decArgo_presDef) = paramPres.fillValue;
      a_parkTemp(a_parkTemp == g_decArgo_tempDef) = paramTemp.fillValue;
      if (~isempty(a_parkSal))
         a_parkSal(a_parkSal == g_decArgo_salDef) = paramSal.fillValue;
      end
      if (~isempty(a_parkSalCor))
         a_parkSalCor(a_parkSalCor == g_decArgo_salDef) = paramSalCor.fillValue;
      end
      if (~isempty(a_parkTempCndc))
         a_parkTempCndc(a_parkTempCndc == g_decArgo_tempDef) = paramTempCndc.fillValue;
      end
      if (~isempty(a_parkC1PhaseDoxy))
         a_parkC1PhaseDoxy(a_parkC1PhaseDoxy == g_decArgo_c1C2PhaseDoxyDef) = paramC1PhaseDoxy.fillValue;
         a_parkC2PhaseDoxy(a_parkC2PhaseDoxy == g_decArgo_c1C2PhaseDoxyDef) = paramC2PhaseDoxy.fillValue;
         a_parkTempDoxy(a_parkTempDoxy == g_decArgo_tempDoxyDef) = paramTempDoxy.fillValue;
         a_parkDoxy(a_parkDoxy == g_decArgo_doxyDef) = paramDoxy.fillValue;
      end
      ...
      for idMeas = 1:length(a_parkPres)
         ...
         measStruct.paramList = [paramPres paramTemp];
         measStruct.paramData = [a_parkPres(idMeas) a_parkTemp(idMeas)];
         if (~isempty(a_parkSal))
            measStruct.paramList = [measStruct.paramList paramSal];
            measStruct.paramData = [measStruct.paramData a_parkSal(idMeas)];
         end
         if (~isempty(a_parkTempCndc))
            measStruct.paramList = [measStruct.paramList paramTempCndc];
            measStruct.paramData = [measStruct.paramData a_parkTempCndc(idMeas)];
         end
         if (~isempty(a_parkC1PhaseDoxy))
            measStruct.paramList = [measStruct.paramList paramC1PhaseDoxy paramC2PhaseDoxy paramTempDoxy paramPpoxDoxy];
            measStruct.paramData = [measStruct.paramData a_parkC1PhaseDoxy(idMeas) a_parkC2PhaseDoxy(idMeas) a_parkTempDoxy(idMeas) a_parkDoxy(idMeas)];
         end
         ...
         if (~isempty(a_parkSalCor))
            measStructAux = measStruct;
            measStructAux.paramList = [paramPres paramSalCor];
            measStructAux.paramData = [a_parkPres(idMeas) a_parkSalCor(idMeas)];
            measStructAux.sensorNumber = 1000;
      ...
      % RPP measurements
      if (~isempty(a_parkSal) && ~isempty(a_parkTempCndc) && ~isempty(a_parkC1PhaseDoxy))
      ...
      elseif (~isempty(a_parkSal) && isempty(a_parkTempCndc) && ~isempty(a_parkC1PhaseDoxy))
         idForMean = find((a_parkPres ~= paramPres.fillValue) & ...
            (a_parkTemp ~= paramTemp.fillValue) & ...
            (a_parkSal ~= paramSal.fillValue) & ...
            (a_parkC1PhaseDoxy ~= paramC1PhaseDoxy.fillValue) & ...
      ...
      if (~isempty(idForMean))
         ...
         if (~isempty(a_parkSal))
            measStruct.paramList = [measStruct.paramList paramSal];
            measStruct.paramData = [measStruct.paramData mean(a_parkSal(idForMean))];
         end
         if (~isempty(a_parkTempCndc))
            measStruct.paramList = [measStruct.paramList paramTempCndc];
            measStruct.paramData = [measStruct.paramData mean(a_parkTempCndc(idForMean))];
         end
         if (~isempty(a_parkC1PhaseDoxy))
            measStruct.paramList = [measStruct.paramList paramC1PhaseDoxy paramC2PhaseDoxy paramTempDoxy paramPpoxDoxy];
            measStruct.paramData = [measStruct.paramData mean(a_parkC1PhaseDoxy(idForMean)) mean(a_parkC2PhaseDoxy(idForMean)) mean(a_parkTempDoxy(idForMean)) mean(a_parkDoxy(idForMean))];
         end
         trajNMeasStruct.tabMeas = [trajNMeasStruct.tabMeas; measStruct];
      ...
      if (~isempty(a_parkSalCor))
         ...
         idForMean = find(~((a_parkPres == paramPres.fillValue) | ...
            (a_parkSalCor == paramSalCor.fillValue)));
         if (~isempty(idForMean))
            measStructAux = get_traj_one_meas_init_struct();
            measStructAux.measCode = g_MC_RPP;

            measStructAux.paramList = [paramPres paramSalCor];
            measStructAux.paramData = [mean(a_parkPres(idForMean)) mean(a_parkSalCor(idForMean))];
            measStructAux.sensorNumber = 1000;
   ...
   if (~isempty(a_nearSurfDate))
      ...
      % convert decoder default values to netCDF fill values
      a_nearSurfPres(a_nearSurfPres == g_decArgo_presDef) = paramPres.fillValue;
      a_nearSurfTemp(a_nearSurfTemp == g_decArgo_tempDef) = paramTemp.fillValue;
      if (~isempty(a_nearSurfSal))
         a_nearSurfSal(a_nearSurfSal == g_decArgo_salDef) = paramSal.fillValue;
      end
      if (~isempty(a_nearSurfSalCor))
         a_nearSurfSalCor(a_nearSurfSalCor == g_decArgo_salDef) = paramSalCor.fillValue;
      end
      if (~isempty(a_nearSurfTempCndc))
         a_nearSurfTempCndc(a_nearSurfTempCndc == g_decArgo_tempDef) = paramTempCndc.fillValue;
      end
      if (~isempty(a_nearSurfC1PhaseDoxy))
         a_nearSurfC1PhaseDoxy(a_nearSurfC1PhaseDoxy == g_decArgo_c1C2PhaseDoxyDef) = paramC1PhaseDoxy.fillValue;
         a_nearSurfC2PhaseDoxy(a_nearSurfC2PhaseDoxy == g_decArgo_c1C2PhaseDoxyDef) = paramC2PhaseDoxy.fillValue;
         a_nearSurfTempDoxy(a_nearSurfTempDoxy == g_decArgo_tempDoxyDef) = paramTempDoxy.fillValue;
         a_nearSurfPpoxDoxy(a_nearSurfPpoxDoxy == g_decArgo_doxyDef) = paramPpoxDoxy.fillValue;
      end
      ...
      for idMeas = 1:length(a_nearSurfPres)
         ...
         % add parameter variables and data to the structure
         measStruct.paramList = [paramPres paramTemp];
         measStruct.paramData = [a_nearSurfPres(idMeas) a_nearSurfTemp(idMeas)];
         if (~isempty(a_nearSurfSal))
            measStruct.paramList = [measStruct.paramList paramSal];
            measStruct.paramData = [measStruct.paramData a_nearSurfSal(idMeas)];
         end
         if (~isempty(a_nearSurfTempCndc))
            measStruct.paramList = [measStruct.paramList paramTempCndc];
            measStruct.paramData = [measStruct.paramData a_nearSurfTempCndc(idMeas)];
         end
         if (~isempty(a_nearSurfC1PhaseDoxy))
            measStruct.paramList = [measStruct.paramList paramC1PhaseDoxy paramC2PhaseDoxy paramTempDoxy paramPpoxDoxy];
            measStruct.paramData = [measStruct.paramData a_nearSurfC1PhaseDoxy(idMeas) a_nearSurfC2PhaseDoxy(idMeas) a_nearSurfTempDoxy(idMeas) a_nearSurfPpoxDoxy(idMeas)];
         end
   ...
   for idProf = 1:length(a_tabProfiles)
      ...
      if (idPres <= length(profile.paramList))
         ...
         if (~isempty(idNotDef))
            ...
            % for PROF or PROF_AUX files
            measStruct.sensorNumber = profile.sensorNumber;
