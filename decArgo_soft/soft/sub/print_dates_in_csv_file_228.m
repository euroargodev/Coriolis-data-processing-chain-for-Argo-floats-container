...
%   a_descProfDate3T, a_descProfDate3TAdj, a_descProfPres3T, ...
%   a_parkDate, a_parkDateAdj, a_parkPres, ...
%   a_parkDate3T, a_parkDate3TAdj, a_parkPres3T, ...
%   a_ascProfDate, a_ascProfDateAdj, a_ascProfPres, ...
%   a_ascProfDate3T, a_ascProfDate3TAdj, a_ascProfPres3T, ...
...
%   a_descProfDate3T    : 3T descending profile dates
%   a_descProfDate3TAdj : 3T descending profile adjusted dates
%   a_descProfPres3T    : 3T descending profile PRES
%   a_parkDate          : drift meas dates
%   a_parkDateAdj       : drift meas adjusted dates
%   a_parkPres          : drift meas PRES
%   a_parkDate3T        : 3T drift meas dates
%   a_parkDate3TAdj     : 3T drift meas adjusted dates
%   a_parkPres3T        : 3T drift meas PRES
...
%   a_ascProfDate3T     : a3T scending profile dates
%   a_ascProfDate3TAdj  : 3T ascending profile adjusted dates
%   a_ascProfPres3T     : 3T ascending profile PRES
...
% ------------------------------------------------------------------------------
% RELEASES :
%   04/25/2024 - RNU - creation
% ------------------------------------------------------------------------------
function print_dates_in_csv_file_228( ...
   a_cycleTimeData, ...
   a_descProfDate, a_descProfDateAdj, a_descProfPres, ...
   a_descProfDate3T, a_descProfDate3TAdj, a_descProfPres3T, ...
   a_parkDate, a_parkDateAdj, a_parkPres, ...
...
idDated = find(a_descProfDate3T ~= g_decArgo_dateDef);
if (~isempty(idDated))
   tabDate = [tabDate a_descProfDate3T(idDated)'];
   if (~any(a_descProfDate3TAdj(idDated) == g_decArgo_dateDef))
      tabDateAdj = [tabDateAdj a_descProfDate3TAdj(idDated)'];
   else
      tabDateAdj = [tabDateAdj ones(1, length(idDated))*g_decArgo_dateDef];
   end
   tabLabel = [tabLabel repmat({'Dated level of 3T descent profile'}, 1, length(idDated))];
   tabPres = [tabPres a_descProfPres3T(idDated)'];
end
...
idDated = find(a_parkDate3T ~= g_decArgo_dateDef);
if (~isempty(idDated))
   tabDate = [tabDate a_parkDate3T(idDated)'];
   if (~any(a_parkDate3TAdj(idDated) == g_decArgo_dateDef))
      tabDateAdj = [tabDateAdj a_parkDate3TAdj(idDated)'];
   else
      tabDateAdj = [tabDateAdj ones(1, length(idDated))*g_decArgo_dateDef];
   end
   tabLabel = [tabLabel repmat({'Park drift 3T meas.'}, 1, length(idDated))];
   tabPres = [tabPres a_parkPres3T(idDated)'];
end
...
idDated = find(a_ascProfDate3T ~= g_decArgo_dateDef);
if (~isempty(idDated))
   tabDate = [tabDate a_ascProfDate3T(idDated)'];
   if (~any(a_ascProfDate3TAdj(idDated) == g_decArgo_dateDef))
      tabDateAdj = [tabDateAdj a_ascProfDate3TAdj(idDated)'];
   else
      tabDateAdj = [tabDateAdj ones(1, length(idDated))*g_decArgo_dateDef];
   end
   tabLabel = [tabLabel repmat({'Dated level of 3T ascent profile'}, 1, length(idDated))];
   tabPres = [tabPres a_ascProfPres3T(idDated)'];
end
