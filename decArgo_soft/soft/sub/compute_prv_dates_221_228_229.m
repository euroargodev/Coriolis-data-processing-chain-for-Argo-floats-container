...
elseif (length(idF2) == 1)
   ...
   if (a_tabTech2(id, 17) > 0)
      ...
      if (~isempty(cycleStartDate))
         if (a_tabTech2(id, 19) == 255) % see 6902893 ##52, 60:68, 73, 74
            firstGroundingDate = fix(cycleStartDate) + a_tabTech2(id, 20)/1440;
         else
   ...
   if (a_tabTech2(id, 17) > 1)
      ...
      if (~isempty(cycleStartDate))
         if (a_tabTech2(id, 24) == 255) % see 6902893 ##52, 60:68, 73, 74
            secondGroundingDate = fix(cycleStartDate) + a_tabTech2(id, 25)/1440;
         else
