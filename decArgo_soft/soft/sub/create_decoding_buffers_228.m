...
% ------------------------------------------------------------------------------
function [o_decodedData] = create_decoding_buffers_228(a_decodedData, a_decoderId)
...
% assign a cyce number to CTD 3T packets
...
idF = find(ismember(tabPackType, [23 24 25]));
for id = idF
...
tabExpNbDesc3T = [a_decodedData.expNbDesc3T];
tabExpNbDrift3T = [a_decodedData.expNbDrift3T];
tabExpNbAsc3T = [a_decodedData.expNbAsc3T];
...
for sesNum = sessionList
   ...
   if (~any(ismember(tabPackType(idForSession), [1 2 3 8 9 10 11 13 23 24 25]))) % transmission after a deep cycle
...
for sesNum = sessionList
   ...
   [completed, deep, ~] = check_buffer(idForCheck, tabPackType, tabExpNbDesc, tabExpNbDesc3T, ...
      tabExpNbDrift, tabExpNbDrift3T, tabExpNbAsc, tabExpNbAsc3T, cyNum, 0, 0);
   delayed = 0;
...
for cyId = 1:length(rankByCycleList)
   ...
   % check current session contents
   [completed, ~, ~] = check_buffer(idForCheck, tabPackType, tabExpNbDesc, tabExpNbDesc3T, ...
      tabExpNbDrift, tabExpNbDrift3T, tabExpNbAsc, tabExpNbAsc3T, cyNum, 0, 1);
   if (completed == 1)
      tabCompleted(idForCheck) = 1;
   else
      tabCompleted(idForCheck) = 0;
   ...
   if (isempty(idForCy))
      ...
      for rankNum = rankNumList
         ...
         if (tabCompleted(idRankCy) == 0)
            [~, ~, why] = check_buffer(idForRankCy, tabPackType, tabExpNbDesc, tabExpNbDesc3T, ...
               tabExpNbDrift, tabExpNbDrift3T, tabExpNbAsc, tabExpNbAsc3T, cyNum, 1, 0);
...
if (~isempty(g_decArgo_outputCsvFileId))
   if (1)
      ...
      if (fId ~= -1)
         ...
         header = '#;Rank;RnkByCycle;Session;SesDeep;Base;Date;DiffDate;Eol;CyNum;IrSession;Deep;Done;Delayed;Completed;Go;PackType;ExpNbDesc;ExpNbDrift;ExpNbAsc;ExpNbDesc3T;ExpNbDrift3T;ExpNbAsc3T;CyNumRaw;ResetDate;ResetFlag;OffsetDate;PackTypeInfo';
         fprintf(fId, '%s\n', header);
...
%   a_tabPackType, a_tabExpNbDesc, a_tabExpNbDesc3T, a_tabExpNbDrift, a_tabExpNbDrift3T, ...
%   a_tabExpNbAsc, a_tabExpNbAsc3T, a_cycleNum, a_whyFlag, a_msgFlag)
%
% INPUT PARAMETERS :
...
%   a_tabExpNbDesc3T  : expected number of descending 3T data packets
%   a_tabExpNbDrift   : expected number of drift data packets
%   a_tabExpNbDrift3T : expected number of drift 3T data packets
%   a_tabExpNbAsc     : expected number of ascending data packets
%   a_tabExpNbAsc3T   : expected number of ascending 3T data packets
...
idPackDesc3T = find(a_tabPackType(a_idForCheck) == 23);
idPackDrift3T = find(a_tabPackType(a_idForCheck) == 24);
idPackAsc3T = find(a_tabPackType(a_idForCheck) == 25);
...
if (~isempty(idPackDesc3T))
   recNbDesc3T = length(idPackDesc3T);
   deep3T = 1;
else
   recNbDesc3T = 0;
end
if (~isempty(idPackDrift3T))
   recNbDrift3T = length(idPackDrift3T);
   deep3T = 1;
else
   recNbDrift3T = 0;
end
if (~isempty(idPackAsc3T))
   recNbAsc3T = length(idPackAsc3T);
   deep3T = 1;
else
   recNbAsc3T = 0;
end
...
if (~isempty(idPackTech3))
   expNbDesc3T = a_tabExpNbDesc3T(a_idForCheck(idPackTech3));
   expNbDrift3T = a_tabExpNbDrift3T(a_idForCheck(idPackTech3));
   expNbAsc3T = a_tabExpNbAsc3T(a_idForCheck(idPackTech3));
   if ((expNbDesc3T == 0) && (expNbDrift3T == 0) && (expNbAsc3T == 0))
      % surface cycle
      completed3T = 1;
   else
      % deep cycle
      if ((recNbDesc3T >= expNbDesc3T) && ...
            (recNbDrift3T >= expNbDrift3T) && ...
            (recNbAsc3T >= expNbAsc3T))
         completed3T = 1;

         if (recNbDesc3T > expNbDesc3T)
            fprintf('BUFF_WARNING: Float #%d Cycle #%3d : %d descending 3T data packets are NOT EXPECTED\n', ...
               g_decArgo_floatNum, a_cycleNum, recNbDesc-expNbDesc);
         end
         if (recNbDrift3T > expNbDrift3T)
            fprintf('BUFF_WARNING: Float #%d Cycle #%3d : %d drift 3T data packets are NOT EXPECTED\n', ...
               g_decArgo_floatNum, a_cycleNum, recNbDrift-expNbDrift);
         end
         if (recNbAsc3T > expNbAsc3T)
            fprintf('BUFF_WARNING: Float #%d Cycle #%3d : %d ascending 3T data packets are NOT EXPECTED\n', ...
               g_decArgo_floatNum, a_cycleNum, recNbAsc-expNbAsc);
         end
      end
      deep3T = 1;
...
if (a_whyFlag && ~o_completed)
   ...
   if (~isempty(idPackTech3))
      if (recNbDesc3T ~= expNbDesc3T)
         if (expNbDesc3T > recNbDesc3T)
            o_whyStr{end+1} = sprintf('%d descending 3T data packets are MISSING', expNbDesc3T-recNbDesc3T);
         else
            o_whyStr{end+1} = sprintf('%d descending 3T data packets are NOT EXPECTED', -(expNbDesc3T-recNbDesc3T));
         end
      end
      if (recNbDrift3T ~= expNbDrift3T)
         if (expNbDrift3T > recNbDrift3T)
            o_whyStr{end+1} = sprintf('%d drift data 3T packets are MISSING', expNbDrift3T-recNbDrift3T);
         else
            o_whyStr{end+1} = sprintf('%d drift data 3T packets are NOT EXPECTED', -(expNbDrift3T-recNbDrift3T));
         end
      end
      if (recNbAsc3T ~= expNbAsc3T)
         if (expNbAsc3T > recNbAsc3T)
            o_whyStr{end+1} = sprintf('%d ascending 3T data packets are MISSING', expNbAsc3T-recNbAsc3T);
         else
            o_whyStr{end+1} = sprintf('%d ascending 3T data packets are NOT EXPECTED', -(expNbAsc3T-recNbAsc3T));
         end
      end
   end
end
...
switch (a_decoderId)
   case {228}
      switch (a_packType)
         ...
         case {23}
            o_packTypeDesc = 'Desc meas 3T';
         case {24}
            o_packTypeDesc = 'Drift meas 3T';
         case {25}
            o_packTypeDesc = 'Asc meas 3T';
         case {26}
            o_packTypeDesc = 'Tech#3 3T';
         case {27}
            o_packTypeDesc = 'Prog#2 3T';
         otherwise
