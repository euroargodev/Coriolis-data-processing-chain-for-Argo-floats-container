...
% ------------------------------------------------------------------------------
function print_in_air_meas_in_csv_file_224_226_227_231( ...
   a_nearSurfDate, a_nearSurfDateAdj, a_nearSurfTransDate, ...
   a_nearSurfPres, a_nearSurfTemp, a_nearSurfSal, a_nearSurfSalCor, a_nearSurfTempCndc, ...
   a_nearSurfC1PhaseDoxy, a_nearSurfC2PhaseDoxy, a_nearSurfTempDoxy, a_nearSurfPpoxDoxy, ...
   a_inAirDate, a_inAirDateAdj, a_inAirTransDate, ...
...
if (~isempty(a_nearSurfDate))
   ...
   if (isempty(a_nearSurfC1PhaseDoxy))
      if (~isempty(a_nearSurfSal) && isempty(a_nearSurfSalCor) && isempty(a_nearSurfTempCndc))
      ...
      elseif (~isempty(a_nearSurfSal) && isempty(a_nearSurfSalCor) && ~isempty(a_nearSurfTempCndc))
      ...
      elseif (~isempty(a_nearSurfSal) && ~isempty(a_nearSurfSalCor) && isempty(a_nearSurfTempCndc))
         ...
         fprintf(g_decArgo_outputCsvFileId, '%d; %d; NearSurf; Description; Float time; UTC time; PRES (dbar); TEMP (degC); PSAL (PSU); PSAL_DynamicCorrection (PSU)\n', ...
            g_decArgo_floatNum, g_decArgo_cycleNum);
         ...
         for idMes = 1:length(a_nearSurfPres)
            ...
            fprintf(g_decArgo_outputCsvFileId, '%d; %d; NearSurf; Near surface meas. #%d; %s (%c); %s; %.1f; %.3f; %.3f; %.3f\n', ...
               g_decArgo_floatNum, g_decArgo_cycleNum, ...
               idMes, mesDateStr, trans, mesDateAdjStr, ...
               a_nearSurfPres(idMes), a_nearSurfTemp(idMes), a_nearSurfSal(idMes), a_nearSurfSalCor(idMes));
         end

      elseif (isempty(a_nearSurfSal) && ~isempty(a_nearSurfSalCor) && isempty(a_nearSurfTempCndc))

         fprintf(g_decArgo_outputCsvFileId, '%d; %d; NearSurf; Description; Float time; UTC time; PRES (dbar); TEMP (degC); PSAL_DynamicCorrection (PSU)\n', ...
            g_decArgo_floatNum, g_decArgo_cycleNum);
         ...
         for idMes = 1:length(a_nearSurfPres)
            ...
            fprintf(g_decArgo_outputCsvFileId, '%d; %d; NearSurf; Near surface meas. #%d; %s (%c); %s; %.1f; %.3f; %.3f\n', ...
               g_decArgo_floatNum, g_decArgo_cycleNum, ...
               idMes, mesDateStr, trans, mesDateAdjStr, ...
               a_nearSurfPres(idMes), a_nearSurfTemp(idMes), a_nearSurfSalCor(idMes));
         end
...
if (~isempty(a_inAirDate))
   ...
   if (isempty(a_inAirC1PhaseDoxy))
      if (~isempty(a_inAirSal) && isempty(a_inAirSalCor) && isempty(a_inAirTempCndc))
      ...
      elseif (~isempty(a_inAirSal) && isempty(a_inAirSalCor) && ~isempty(a_inAirTempCndc))
      ...
      elseif (~isempty(a_inAirSal) && ~isempty(a_inAirSalCor) && isempty(a_inAirTempCndc))
         ...
         fprintf(g_decArgo_outputCsvFileId, '%d; %d; InAir; Description; Float time; UTC time; PRES (dbar); TEMP (degC); PSAL (PSU); PSAL_DynamicCorrection (PSU)\n', ...
            g_decArgo_floatNum, g_decArgo_cycleNum);
         ...
         for idMes = 1:length(a_inAirPres)
            ...
            fprintf(g_decArgo_outputCsvFileId, '%d; %d; InAir; In air meas. #%d; %s (%c); %s; %.1f; %.3f; %.3f; %.3f\n', ...
               g_decArgo_floatNum, g_decArgo_cycleNum, ...
               idMes, mesDateStr, trans, mesDateAdjStr, ...
               a_inAirPres(idMes), a_inAirTemp(idMes), a_inAirSal(idMes), a_inAirSalCor(idMes));
         end
      
      elseif (isempty(a_inAirSal) && ~isempty(a_inAirSalCor) && isempty(a_inAirTempCndc))

         fprintf(g_decArgo_outputCsvFileId, '%d; %d; InAir; Description; Float time; UTC time; PRES (dbar); TEMP (degC); PSAL_DynamicCorrection (PSU)\n', ...
            g_decArgo_floatNum, g_decArgo_cycleNum);
         ...
         for idMes = 1:length(a_inAirPres)
            ...
            fprintf(g_decArgo_outputCsvFileId, '%d; %d; InAir; In air meas. #%d; %s (%c); %s; %.1f; %.3f; %.3f\n', ...
               g_decArgo_floatNum, g_decArgo_cycleNum, ...
               idMes, mesDateStr, trans, mesDateAdjStr, ...
               a_inAirPres(idMes), a_inAirTemp(idMes), a_inAirSalCor(idMes));
         end
