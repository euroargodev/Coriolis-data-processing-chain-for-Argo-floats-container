...
% ------------------------------------------------------------------------------
function [o_decodedData] = create_decoding_buffers_229(a_decodedData, a_decoderId)
...
% assign a cyce number to CTD 2T packets
tabPackType = [a_decodedData.packType];
tabCyNum = [a_decodedData.cyNumRaw];
tabDate = [a_decodedData.fileDate];
idCy = find(tabCyNum ~= -1);
cyNumList = tabCyNum(idCy);
cyNumDate = tabDate(idCy);
[cyNumDate, idUnique, ~] = unique(cyNumDate);
cyNumList = cyNumList(idUnique);
idF = find(ismember(tabPackType, [28 29]));
for id = idF
   [~, idMin] = min(abs(a_decodedData(id).fileDate-cyNumDate));
   a_decodedData(id).cyNumRaw = cyNumList(idMin);
   decData = a_decodedData(id).decData{:};
   decData(2) = cyNumList(idMin);
   a_decodedData(id).decData = {decData};
end
...
tabExpNbDesc2T = [a_decodedData.expNbDesc2T];
tabExpNbDrift2T = [a_decodedData.expNbDrift2T];
tabExpNbAsc2T = [a_decodedData.expNbAsc2T];
...
for sesNum = sessionList
   ...
   if (~any(ismember(tabPackType(idForSession), [1 2 3 8 9 10 11 13 28 29]))) % transmission after a deep cycle
...
for sesNum = sessionList
   ...
   [completed, deep, ~] = check_buffer(idForCheck, tabPackType, tabExpNbDesc, tabExpNbDesc2T, ...
      tabExpNbDrift, tabExpNbDrift2T, tabExpNbAsc, tabExpNbAsc2T, cyNum, 0, 0);
   delayed = 0;
...
if (ismember(g_decArgo_floatNum, [ ...
   ...
   switch g_decArgo_floatNum
      case 6990627
         % remove the last packet of all incompleted sessions
         idF = find(tabCompleted == 0);
         rankList = -1;
         for id = idF
            if (~ismember(tabRank(id), rankList))
               idForRank = find(tabRank == tabRank(id));
               tabRank(idForRank(end)) = -1;
               tabRankByCycle(idForRank(end)) = -1;
               tabCompleted(idForRank) = 1;
               rankList = [rankList tabRank(id)];
            end
         end
         tabDelayed(tabDelayed == 2) = 0;
         tabGo(tabGo == 2) = 1;
   end
...
for cyId = 1:length(rankByCycleList)
   ...
   % check current session contents
   [completed, ~, ~] = check_buffer(idForCheck, tabPackType, tabExpNbDesc, tabExpNbDesc2T, ...
      tabExpNbDrift, tabExpNbDrift2T, tabExpNbAsc, tabExpNbAsc2T, cyNum, 0, 1);
   if (completed == 1)
      tabCompleted(idForCheck) = 1;
   else
      tabCompleted(idForCheck) = 0;
   ...
   if (isempty(idForCy))
      ...
      for rankNum = rankNumList
         ...
         if (tabCompleted(idRankCy) == 0)
            [~, ~, why] = check_buffer(idForRankCy, tabPackType, tabExpNbDesc, tabExpNbDesc2T, ...
               tabExpNbDrift, tabExpNbDrift2T, tabExpNbAsc, tabExpNbAsc2T, cyNum, 1, 0);
...
if (~isempty(g_decArgo_outputCsvFileId))
   if (1)
      ...
      if (fId ~= -1)
         ...
         header = '#;Rank;RnkByCycle;Session;SesDeep;Base;Date;DiffDate;Eol;CyNum;IrSession;Deep;Done;Delayed;Completed;Go;PackType;ExpNbDesc;ExpNbDrift;ExpNbAsc;ExpNbDesc2T;ExpNbDrift2T;ExpNbAsc2T;CyNumRaw;ResetDate;ResetFlag;OffsetDate;PackTypeInfo';
         fprintf(fId, '%s\n', header);
         ...
         for idL = 1:length(tabPackType)
            ...
            fprintf(fId, '%d;%d;%d;%d;%d;%d;%s;%s;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%s;%d;%d;%s\n', ...
               idL, ...
               tabRank(idL), ...
               tabRankByCycle(idL), ...
               tabSession(idL), ...
...
%
% SYNTAX :
% [o_completed, o_deep, o_whyStr] = check_buffer(a_idForCheck, ...
%   a_tabPackType, a_tabExpNbDesc, a_tabExpNbDesc2T, a_tabExpNbDrift, a_tabExpNbDrift2T, ...
%   a_tabExpNbAsc, a_tabExpNbAsc2T, a_cycleNum, a_whyFlag, a_msgFlag)
%
% INPUT PARAMETERS :
...
%   a_tabExpNbDesc2T  : expected number of descending 2T data packets
%   a_tabExpNbDrift   : expected number of drift data packets
%   a_tabExpNbDrift2T : expected number of drift 2T data packets
%   a_tabExpNbAsc     : expected number of ascending data packets
%   a_tabExpNbAsc2T   : expected number of ascending 2T data packets
...
% ------------------------------------------------------------------------------
function [o_completed, o_deep, o_whyStr] = check_buffer(a_idForCheck, ...
   a_tabPackType, a_tabExpNbDesc, a_tabExpNbDesc2T, a_tabExpNbDrift, a_tabExpNbDrift2T, ...
   a_tabExpNbAsc, a_tabExpNbAsc2T, a_cycleNum, a_whyFlag, a_msgFlag)
...
idPackTech3 = find(a_tabPackType(a_idForCheck) == 26);
idPackProg = find(a_tabPackType(a_idForCheck) == 5);
idPackProg2 = find(a_tabPackType(a_idForCheck) == 27);
idPackDesc = find(a_tabPackType(a_idForCheck) == 1);
idPackDrift = find(a_tabPackType(a_idForCheck) == 2);
idPackAsc = find(a_tabPackType(a_idForCheck) == 3);
idPackDesc2T = find(a_tabPackType(a_idForCheck) == 28);
idPackDrift2T = '';
idPackAsc2T = find(a_tabPackType(a_idForCheck) == 29);

if ((length(idPackTech1) > 1) || (length(idPackTech2) > 1) || (length(idPackTech3) > 1) || ...
      (length(idPackProg) > 1) || (length(idPackProg2) > 1))
   ...
   if ((length(idPackTech3) > 1) && a_msgFlag)
      fprintf('ERROR: Float #%d Cycle #%3d : multiple (%d) Tech#3 packet in the buffer\n', ...
         g_decArgo_floatNum, a_cycleNum, length(idPackTech3));
   end
   ...
   if ((length(idPackProg2) > 1) && a_msgFlag)
      fprintf('ERROR: Float #%d Cycle #%3d : multiple (%d) Prog#2 packet in the buffer\n', ...
         g_decArgo_floatNum, a_cycleNum, length(idPackProg2));
   end
   return
end
...
if (~isempty(idPackDesc2T))
   recNbDesc2T = length(idPackDesc2T);
   deep2T = 1;
else
   recNbDesc2T = 0;
end
if (~isempty(idPackDrift2T))
   recNbDrift2T = length(idPackDrift2T);
   deep2T = 1;
else
   recNbDrift2T = 0;
end
if (~isempty(idPackAsc2T))
   recNbAsc2T = length(idPackAsc2T);
   deep2T = 1;
else
   recNbAsc2T = 0;
end
...
if (~isempty(idPackTech3))
   expNbDesc2T = a_tabExpNbDesc2T(a_idForCheck(idPackTech3));
   expNbDrift2T = a_tabExpNbDrift2T(a_idForCheck(idPackTech3));
   expNbAsc2T = a_tabExpNbAsc2T(a_idForCheck(idPackTech3));
   if ((expNbDesc2T == 0) && (expNbDrift2T == 0) && (expNbAsc2T == 0))
      % surface cycle
      completed2T = 1;
   else
      % deep cycle
      if ((recNbDesc2T >= expNbDesc2T) && ...
            (recNbDrift2T >= expNbDrift2T) && ...
            (recNbAsc2T >= expNbAsc2T))
         completed2T = 1;

         if (recNbDesc2T > expNbDesc2T)
            fprintf('BUFF_WARNING: Float #%d Cycle #%3d : %d descending 2T data packets are NOT EXPECTED\n', ...
               g_decArgo_floatNum, a_cycleNum, recNbDesc-expNbDesc);
         end
         if (recNbDrift2T > expNbDrift2T)
            fprintf('BUFF_WARNING: Float #%d Cycle #%3d : %d drift 2T data packets are NOT EXPECTED\n', ...
               g_decArgo_floatNum, a_cycleNum, recNbDrift-expNbDrift);
         end
         if (recNbAsc2T > expNbAsc2T)
            fprintf('BUFF_WARNING: Float #%d Cycle #%3d : %d ascending 2T data packets are NOT EXPECTED\n', ...
               g_decArgo_floatNum, a_cycleNum, recNbAsc-expNbAsc);
         end
      end
      deep2T = 1;
...
if (~isempty(idPackTech2) && ~isempty(idPackTech3))
   if ((completed == 1) && (completed2T == 1))
      o_completed = 1;
   end
   if ((deep == 1) || (deep2T == 1))
      o_deep = 1;
   end
elseif (~isempty(idPackTech2))
   o_completed = completed;
   o_deep = deep;
elseif (~isempty(idPackTech3))
   o_completed = completed2T;
   o_deep = deep2T;
end
...
if (a_whyFlag && ~o_completed)
   ...
   if (~isempty(idPackTech3))
      if (recNbDesc2T ~= expNbDesc2T)
         if (expNbDesc2T > recNbDesc2T)
            o_whyStr{end+1} = sprintf('%d descending 2T data packets are MISSING', expNbDesc2T-recNbDesc2T);
         else
            o_whyStr{end+1} = sprintf('%d descending 2T data packets are NOT EXPECTED', -(expNbDesc2T-recNbDesc2T));
         end
      end
      if (recNbDrift2T ~= expNbDrift2T)
         if (expNbDrift2T > recNbDrift2T)
            o_whyStr{end+1} = sprintf('%d drift data 2T packets are MISSING', expNbDrift2T-recNbDrift2T);
         else
            o_whyStr{end+1} = sprintf('%d drift data 2T packets are NOT EXPECTED', -(expNbDrift2T-recNbDrift2T));
         end
      end
      if (recNbAsc2T ~= expNbAsc2T)
         if (expNbAsc2T > recNbAsc2T)
            o_whyStr{end+1} = sprintf('%d ascending 2T data packets are MISSING', expNbAsc2T-recNbAsc2T);
         else
            o_whyStr{end+1} = sprintf('%d ascending 2T data packets are NOT EXPECTED', -(expNbAsc2T-recNbAsc2T));
         end
      end
   end
end
   ...
   case {229}
      switch (a_packType)
         ...
         case 7
            o_packTypeDesc = 'Pump';
         case {28}
            o_packTypeDesc = 'Desc meas 2T';
         case {29}
            o_packTypeDesc = 'Asc meas 2T';
         case {26}
            o_packTypeDesc = 'Tech#3 2T';
         case {27}
            o_packTypeDesc = 'Prog#2 2T';
         otherwise
