...
if (~isempty(g_decArgo_calibInfo) && isfield(g_decArgo_calibInfo, 'OPTODE') && isfield(g_decArgo_calibInfo.OPTODE, 'TabDoxyCoef'))
   ...
   if (~isempty(tabDoxyCoef))
      ...
      if (isempty(find((size(tabDoxyCoef) == [2 7]) ~= 1, 1)))
         for idC = 1:4
            fprintf(g_decArgo_outputCsvFileId, '%d; %d; Calib; %s; %e\n', ...
               g_decArgo_floatNum, -1, ...
      ...
      elseif (isempty(find((size(tabDoxyCoef) == [7 7]) ~= 1, 1)))
         % Arvor Deep 2DO (5.77)
         for idC = 1:4
            fprintf(g_decArgo_outputCsvFileId, '%d; %d; Calib; %s; %e\n', ...
               g_decArgo_floatNum, -1, ...
         ...
         coef = 'ABCDEF';
         for idC = 1:length(coef)
            fprintf(g_decArgo_outputCsvFileId, '%d; %d; Calib; %s; %e\n', ...
               g_decArgo_floatNum, -1, ...
               ['ArodFtOptodeTemp' coef(idC)], tabDoxyCoef(3, idC));
         end
         for idC = 1:3
            fprintf(g_decArgo_outputCsvFileId, '%d; %d; Calib; %s; %e\n', ...
               g_decArgo_floatNum, -1, ...
               ['ArodFtOptodeC' num2str(idC-1)], tabDoxyCoef(4, idC));
         end
         for idC = 1:5
            fprintf(g_decArgo_outputCsvFileId, '%d; %d; Calib; %s; %e\n', ...
               g_decArgo_floatNum, -1, ...
               ['ArodFtOptodeD' num2str(idC-1)], tabDoxyCoef(5, idC));
         end
         fprintf(g_decArgo_outputCsvFileId, '%d; %d; Calib; %s; %e\n', ...
            g_decArgo_floatNum, -1, ...
            'ArodFtOptodeE0', tabDoxyCoef(6, 1));
         fprintf(g_decArgo_outputCsvFileId, '%d; %d; Calib; %s; %e\n', ...
            g_decArgo_floatNum, -1, ...
            'ArodFtOptodeCp', tabDoxyCoef(7, 1));
      end
   end
end
