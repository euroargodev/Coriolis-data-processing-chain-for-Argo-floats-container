...
%
% SYNTAX :
%  [o_ramsesData] = decode_apmt_ramsesV2_data(a_data, a_lastByteNum, a_inputFilePathName)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function [o_ramsesData] = decode_apmt_ramsesV2_data(a_data, a_lastByteNum, a_inputFilePathName)
...
bitList = [ ...
   ...
   {[]} ...
   {[]} ...
   {[repmat(16, 1, 7) 8 16 8]} ... % the number of channels is provided in the data (bit pattern will be updated later)
   {[]} ...
   {[]} ... % unused but to be consistent with g_decArgo_cts5Treat_SS (so that 'DW' is g_decArgo_cts5Treat_DW == 7)
   {[repmat(16, 1, 7) 8 16 8]} ... % the number of channels is provided in the data (bit pattern will be updated later)
   ];
...
signedList = [ ...
   ...
   {[]} ...
   {[]} ...
   {zeros(1, 10)} ... % the number of channels is provided in the data (list of signed type parameters will be updated later)
   {[]} ...
   {[]} ... % unused but to be consistent with g_decArgo_cts5Treat_SS (so that 'DW' is g_decArgo_cts5Treat_DW == 7)
   {zeros(1, 10)} ... % the number of channels is provided in the data (list of signed type parameters will be updated later)
   ];
...
while (currentByte <= lastByteNum)
   ...
   if ((currentPhaseNum ~= -1) && (currentTreatNum ~= -1))
      ...
      if (ismember(currentTreatNum, [g_decArgo_cts5Treat_RW g_decArgo_cts5Treat_DW]))
         ...
         data(9) = rawData(9);
         data(10) = rawData(10);
         data(11:10+nbChannels) = rawData(11:end);
      else
