...
%
% SYNTAX :
%  [o_decodedData] = decode_prv_data_ir_sbd_229( ...
...
% ------------------------------------------------------------------------------
function [o_decodedData] = decode_prv_data_ir_sbd_229( ...
   a_tabData, a_sbdFileName, a_sbdFileDate, a_launchDate)
...
switch (packType)
   ...
   case 26
      ...
      decodedData.cyNumRaw = cycleNum;
      decodedData.expNbDesc2T = tabTech3(8);
      decodedData.expNbDrift2T = tabTech3(9);
      decodedData.expNbAsc2T = tabTech3(10);
   ...
   case {1, 2, 3}
      ...
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   case {28, 29}
      % CTD packets dedicated to 2T prototype
      ...
      % there are 6 PTS measurements from each sensor per packet
      ...
      for idBin = 1:6
         ...
         presRbr = ctdValues(7*(idBin-1)+4);
         tempRbr = ctdValues(7*(idBin-1)+5);
         psalRbr = ctdValues(7*(idBin-1)+6);
         tempCndcRbr = ctdValues(7*(idBin-1)+7);

         presSbe61 = ctdValues(7*(idBin-1)+8);
         tempSbe61 = ctdValues(7*(idBin-1)+9);
         psalSbe61 = ctdValues(7*(idBin-1)+10);
      ...
      dataCTD = [packType nan tabDate' ones(1, length(tabDate))*-1 ...
         tabPresRbr' tabTempRbr' tabPsalRbr' tabTempCndcRbr' ...
         tabPresSbe61' tabTempSbe61' tabPsalSbe61' a_sbdFileDate];
   ...
   case 5
      ...
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   case 27
      % parameter paket dedicated to 2T prototype
      ...
      % get item bits
      tabParam2T = get_bits(firstBit, tabNbBits, msgData);
      
      cycleNum = tabParam2T(7)-1;

      % cbar => dbar conversion
      tabParam2T(14) = tabParam2T(14)/10;
      tabParam2T(15) = tabParam2T(15)/10;
      tabParam2T(20) = tabParam2T(20)/10;
      tabParam2T(21) = tabParam2T(21)/10;
      tabParam2T(22) = tabParam2T(22)/10;
      tabParam2T(23) = tabParam2T(23)/10;
      tabParam2T(24) = tabParam2T(24)/10;
      
      % compute float time
      floatTime = datenum(sprintf('%02d%02d%02d%02d%02d%02d', tabParam2T(1:6)), 'HHMMSSddmmyy') - g_decArgo_janFirst1950InMatlab;
      
      floatParam2T = [packType cycleNum tabParam2T' floatTime a_sbdFileDate];
      
      decodedData.decData = {floatParam2T};
      decodedData.cyNumRaw = cycleNum;
