...
%
% SYNTAX :
%  [o_tabProfiles] = merge_profile_cts5_usea_eco_126_to_130_133_to_136_138_139_141(a_tabProfiles)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function [o_tabProfiles] = merge_profile_cts5_usea_eco_126_to_130_133_to_136_138_139_141(a_tabProfiles)
...
elseif (ismember('ECO_FLNTU', g_decArgo_sensorMountedOnFloat))
   [o_tabProfiles] = merge_profiles_eco_flntu(a_tabProfiles);
end
...
%
% SYNTAX :
%  [o_tabProfile] = merge_profiles_eco_flntu(a_tabProfiles)
%
% INPUT PARAMETERS :
%   a_tabProfiles : input profiles to merge
%
...
% ------------------------------------------------------------------------------
function [o_tabProfiles] = merge_profiles_eco_flntu(a_tabProfiles)
...
paramFluorescenceChlaStDev = get_netcdf_param_attributes('FLUORESCENCE_CHLA_STD');
paramSideScatteringTurbidityStDev = get_netcdf_param_attributes('SIDE_SCATTERING_TURBIDITY_STD');
paramPresMed = get_netcdf_param_attributes('PRES_MED');
paramFluorescenceChlaMed = get_netcdf_param_attributes('FLUORESCENCE_CHLA_MED');
paramSideScatteringTurbidityMed = get_netcdf_param_attributes('SIDE_SCATTERING_TURBIDITY_MED');
...
for idDir = 1:length(uDir)
      ...
      switch (a_tabProfiles(idProf).treatType)
         case {g_decArgo_treatRaw, g_decArgo_treatAverage, g_decArgo_treatDecimatedRaw}
            ...
            finalData = [finalData; ...
               data, ...
               ones(size(data, 1), 1)*paramFluorescenceChlaStDev.fillValue, ...
               ones(size(data, 1), 1)*paramSideScatteringTurbidityStDev.fillValue, ...
               ones(size(data, 1), 1)*paramPresMed.fillValue, ...
               ones(size(data, 1), 1)*paramFluorescenceChlaMed.fillValue, ...
               ones(size(data, 1), 1)*paramSideScatteringTurbidityMed.fillValue, ...
               ];
   ...
   paramList = [ ...
      paramPres paramFluorescenceChla paramSideScatteringTurbidity ...
      paramFluorescenceChlaStDev paramSideScatteringTurbidityStDev ...
      paramPresMed paramFluorescenceChlaMed paramSideScatteringTurbidityMed ...
      ];
