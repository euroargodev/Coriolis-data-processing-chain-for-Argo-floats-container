...
tabTechDelayed = []; % for TECH and TECH_AUX files of a previous Cycle
   ...
   switch (evtNum)
      ...
      case 9000
         if (~isempty(g_decArgo_outputCsvFileId))
            ...
            tabEvt = [tabEvt evt];
            if (data ~= missionNum)
               fprintf('WARNING: Float #%d: Inconsistent mission number (%d while %d is expected) in TECH file: %s\n', ...
                  g_decArgo_floatNum, ...
                  data, missionNum, a_fileName);
            end
            ...
            tabEvt = [tabEvt evt];
            if (data ~= cycleNum)
               fprintf('WARNING: Float #%d: Inconsistent cycle number (%d while %d is expected) in TECH file: %s\n', ...
                  g_decArgo_floatNum, ...
                  data, cycleNum, a_fileName);
            end
