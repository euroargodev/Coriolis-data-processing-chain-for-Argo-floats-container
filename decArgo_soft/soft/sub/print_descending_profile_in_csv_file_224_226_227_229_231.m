...
% ------------------------------------------------------------------------------
function print_descending_profile_in_csv_file_224_226_227_229_231( ...
   a_descProfDate, a_descProfDateAdj, ...
   a_descProfPres, a_descProfTemp, a_descProfSal, a_descProfSalCor, a_descProfTempCndc, ...
   a_descProfC1PhaseDoxy, a_descProfC2PhaseDoxy, a_descProfTempDoxy, a_descProfDoxy)
...
if (~isempty(a_descProfPres))
   ...
   if (isempty(a_descProfC1PhaseDoxy))
      if (~isempty(a_descProfSal) && isempty(a_descProfSalCor) && isempty(a_descProfTempCndc))
      ...
      elseif (~isempty(a_descProfSal) && isempty(a_descProfSalCor) && ~isempty(a_descProfTempCndc))
      ...
      elseif (~isempty(a_descProfSal) && ~isempty(a_descProfSalCor) && isempty(a_descProfTempCndc))
         ...
         fprintf(g_decArgo_outputCsvFileId, '%d; %d; DescProf; Description; Float time; UTC time; PRES (dbar); TEMP (degC); PSAL (PSU); PSAL_DynamicCorrection (PSU)\n', ...
            g_decArgo_floatNum, g_decArgo_cycleNum);
         ...
         for idMes = length(a_descProfPres):-1:1
            ...
            fprintf(g_decArgo_outputCsvFileId, '%d; %d; DescProf; Desc. profile meas. #%d; %s; %s; %.1f; %.3f; %.3f; %.3f\n', ...
               g_decArgo_floatNum, g_decArgo_cycleNum, ...
               length(a_descProfPres)-idMes+1, mesDateStr, mesDateAdjStr, ...
               a_descProfPres(idMes), a_descProfTemp(idMes), a_descProfSal(idMes), a_descProfSalCor(idMes));
         end

      elseif (isempty(a_descProfSal) && ~isempty(a_descProfSalCor) && isempty(a_descProfTempCndc))

         fprintf(g_decArgo_outputCsvFileId, '%d; %d; DescProf; Description; Float time; UTC time; PRES (dbar); TEMP (degC); PSAL_DynamicCorrection (PSU)\n', ...
            g_decArgo_floatNum, g_decArgo_cycleNum);
         ...
         for idMes = length(a_descProfPres):-1:1
            ...
            fprintf(g_decArgo_outputCsvFileId, '%d; %d; DescProf; Desc. profile meas. #%d; %s; %s; %.1f; %.3f; %.3f\n', ...
               g_decArgo_floatNum, g_decArgo_cycleNum, ...
               length(a_descProfPres)-idMes+1, mesDateStr, mesDateAdjStr, ...
               a_descProfPres(idMes), a_descProfTemp(idMes), a_descProfSalCor(idMes));
         end
