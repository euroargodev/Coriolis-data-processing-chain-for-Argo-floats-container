...
% ------------------------------------------------------------------------------
function [o_DOXY] = compute_DOXY_40x( ...
   a_C1PHASE_DOXY, a_C2PHASE_DOXY, a_TEMP_DOXY, ...
   a_C1PHASE_DOXY_fillValue, a_C2PHASE_DOXY_fillValue, a_TEMP_DOXY_fillValue, ...
   a_PRES, a_TEMP, a_PSAL, ...
   a_PRES_fillValue, a_TEMP_fillValue, a_PSAL_fillValue, ...
...
if (isempty(g_decArgo_calibInfo))
   fprintf('WARNING: Float #%d Cycle #%d: DOXY calibration coefficients are missing - DOXY data set to fill value in ''%c'' profile of OPTODE sensor\n', ...
      g_decArgo_floatNum, ...
      g_decArgo_cycleNum, ...
      a_direction);
   return
elseif ((isfield(g_decArgo_calibInfo, 'OPTODE')) && (isfield(g_decArgo_calibInfo.OPTODE, 'TabDoxyCoef')))
   ...
   if (~isempty(find((size(tabDoxyCoef) == [2 7]) ~= 1, 1)))
      fprintf('ERROR: Float #%d Cycle #%d: DOXY calibration coefficients are inconsistent - DOXY data set to fill value in ''%c'' profile of OPTODE sensor\n', ...
         g_decArgo_floatNum, ...
         g_decArgo_cycleNum, ...
         a_direction);
      return
   ...
   fprintf('ERROR: Float #%d Cycle #%d: inconsistent DOXY calibration coefficients - DOXY data set to fill value in ''%c'' profile of OPTODE sensor\n', ...
         g_decArgo_floatNum, ...
         g_decArgo_cycleNum, ...
         a_direction);
   return
