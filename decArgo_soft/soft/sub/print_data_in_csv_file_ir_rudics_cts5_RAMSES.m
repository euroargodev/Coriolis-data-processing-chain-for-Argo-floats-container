...
%
% SYNTAX :
 % print_data_in_csv_file_ir_rudics_cts5_RAMSES( ...
 %   a_ramsesData, a_ramses2Data, a_ramsesV2Data, a_ramses2V2Data)
%
% INPUT PARAMETERS :
...
% ------------------------------------------------------------------------------
function print_data_in_csv_file_ir_rudics_cts5_RAMSES( ...
   a_ramsesData, a_ramses2Data, a_ramsesV2Data, a_ramses2V2Data)
...
if (isempty(a_ramsesData) && isempty(a_ramses2Data) && isempty(a_ramsesV2Data) && isempty(a_ramses2V2Data))
   return
end
...
if (~isempty(a_ramsesV2Data))

   fileTypeStr = 'Data_apmt';
   sensorNum = 'SENSOR_14';
   sensorName = 'Ramses V2';
   phasePrev = '';
   for idP = 1:length(a_ramsesV2Data)

      dataStruct = a_ramsesV2Data{idP};
      ...
      switch (treat)
         case {'(RW)', '(DW)'}
            fprintf(g_decArgo_outputCsvFileId, ['%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; ' ...
               'PRES (dbar); INTEGRATION_TIME (msec); ' ...
               'PRE_PRES (dbar); POST_PRES (dbar); PRE_INCLINATION (degree); POST_INCLINATION (degree); TIMESTAMP (sec); DARK_AVERAGE; Number of channels'], ...
               g_decArgo_floatNum, g_decArgo_cycleNumFloatStr, g_decArgo_patternNumFloatStr, ...
               fileTypeStr, phase, sensorNum, sensorName);
            fprintf(g_decArgo_outputCsvFileId, '; RAW_DOWNWELLING_IRRADIANCE_%d (count)', 1:size(data, 2)-9);
            fprintf(g_decArgo_outputCsvFileId, '\n');
            outputFmt = ['%s; %s;%.1f;%d;%.2f;%.2f;%.2f;%.2f;%.2f;%d;%d' repmat(';%d', 1, size(data, 2)-9)];
            if (strcmp(treat, '(RW)'))
               measType = 'raw';
            elseif (strcmp(treat, '(DW)'))
               measType = 'decimated raw';
...
if (~isempty(a_ramses2V2Data))

   fileTypeStr = 'Data_apmt';
   sensorNum = 'SENSOR_21';
   sensorName = 'Ramses2 V2';
   phasePrev = '';
   for idP = 1:length(a_ramses2V2Data)

      dataStruct = a_ramses2V2Data{idP};
