...
%   a_opusLightData, a_opusLightV2Data, a_opusBlackData, a_opusBlackV2Data)
%
% INPUT PARAMETERS :
...
%   a_opusBlackV2Data : CTS5-USEA OPUS-BLACK V2 data
%
% OUTPUT PARAMETERS :
%
% EXAMPLES :
...
% ------------------------------------------------------------------------------
function print_data_in_csv_file_ir_rudics_cts5_OPUS( ...
   a_opusLightData, a_opusLightV2Data, a_opusBlackData, a_opusBlackV2Data)
...
if (isempty(a_opusLightData) && isempty(a_opusLightV2Data) && ...
      isempty(a_opusBlackData) && isempty(a_opusBlackV2Data))
   return
end
...
if (~isempty(a_opusLightData))
   ...
   for idP = 1:length(a_opusLightData)
      ...
      switch (treat)
         case {'(RW)', '(DW)'}
            header1 = '%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; PRES (dbar); SPECTRUM_TYPE_NITRATE (2:calibrated, 4:raw); AVERAGING_NITRATE (count); FLASH_COUNT_NITRATE (count); TEMP_SPECTROPHOTOMETER_NITRATE (degC)';
...
if (~isempty(a_opusLightV2Data))
   ...
   for idP = 1:length(a_opusLightV2Data)
      ...
      switch (treat)
         case {'(RW)', '(DW)'}
            header1 = '%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; PRES (dbar); SPECTRUM_TYPE_NITRATE (2:calibrated, 4:raw); AVERAGING_NITRATE (count); FLASH_COUNT_NITRATE (count); TEMP_SPECTROPHOTOMETER_NITRATE (degC); LAMP_REFERENCE_1';
            header2 = [' ;nb channels full' sprintf('; UV_INTENSITY_FULL_NITRATE_%d (count)', 1:nbF)];
            header3 = [' ;nb channels binned' sprintf('; UV_INTENSITY_BINNED_NITRATE_%d (count)', 1:nbB)];
            fprintf(g_decArgo_outputCsvFileId, [header1 header2 header3 '\n'], ...
               g_decArgo_floatNum, g_decArgo_cycleNumFloatStr, g_decArgo_patternNumFloatStr, ...
               fileTypeStr, phase, sensorNum, sensorName);
            format1 = [' %s; %s;%.1f;%d;%d;%d;%.4f;%d'];
            format2 = [';%d' repmat(';%d', 1, nbF)];
            format3 = [';%d' repmat(';%d', 1, nbB)];
            outputFmt = [format1 format2 format3];
            if (strcmp(treat, '(RW)'))
      ...
      for idL = 1:size(data, 1)
         fprintf(g_decArgo_outputCsvFileId, ['%d; %s; %s; %s; %s; %s; %s; meas #%4d (%s); ' outputFmt '\n'], ...
            ...
            measNum, measType, ...
            julian_2_gregorian_dec_argo(data(idL, 1)), julian_2_gregorian_dec_argo(datesAdj(idL)), ...
            data(idL, 2:8),  data(idL, 9:9+nbF-1), data(idL, 264:265+nbB-1));
         measNum = measNum + 1;
      end
   end
end
...
if (~isempty(a_opusBlackData))
   ...
   for idP = 1:length(a_opusBlackData)
      ...
      switch (treat)
         case {'(RW)', '(DW)'}
            fprintf(g_decArgo_outputCsvFileId, '%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; PRES (dbar); AVERAGING_NITRATE (count); FLASH_COUNT_NITRATE (count); TEMP_SPECTROPHOTOMETER_NITRATE (degC); UV_INTENSITY_DARK_NITRATE_AVG (count); UV_INTENSITY_DARK_NITRATE_SD (count)\n', ...
               g_decArgo_floatNum, g_decArgo_cycleNumFloatStr, g_decArgo_patternNumFloatStr, ...
               fileTypeStr, phase, sensorNum, sensorName);
...
if (~isempty(a_opusBlackV2Data))
   
   fileTypeStr = 'Data_apmt';
   sensorNum = 'SENSOR_15';
   sensorName = 'Opus (BLACK V2)';
   phasePrev = '';
   for idP = 1:length(a_opusBlackV2Data)
      
      dataStruct = a_opusBlackV2Data{idP};
      ...
      switch (treat)
         case {'(RW)', '(DW)'}
            fprintf(g_decArgo_outputCsvFileId, '%d; %s; %s; %s; %s; %s; %s; -; Float time; Adj. float time; PRES (dbar); AVERAGING_NITRATE (count); FLASH_COUNT_NITRATE (count); TEMP_SPECTROPHOTOMETER_NITRATE (degC); LAMP_REFERENCE_1; UV_INTENSITY_DARK_NITRATE_AVG (count); UV_INTENSITY_DARK_NITRATE_SD (count)\n', ...
               g_decArgo_floatNum, g_decArgo_cycleNumFloatStr, g_decArgo_patternNumFloatStr, ...
               fileTypeStr, phase, sensorNum, sensorName);
            outputFmt = '%s; %s;%.1f;%d;%d;%.4f;%d;%g;%g';
            if (strcmp(treat, '(RW)'))
               measType = 'raw';
            elseif (strcmp(treat, '(DW)'))
               measType = 'decimated raw';
